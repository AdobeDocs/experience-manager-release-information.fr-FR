...
aem Cumulative Fix Pack 6.3.3.5 dépend de AEM 6.3 Service Pack 3. Par conséquent, vous devez installer AEM Cumulative Fix Pack 6.3.3.x après avoir installé AEM 6.3 Service Pack 3. Pour obtenir des instructions sur l’installation, consultez les [Notes de mise à jour du service Pack 3 d’AEM 6.3](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
* (Configurations du Cloud Service) La case &quot;hérité de&quot; qui s’affiche au niveau racine doit être supprimée. NPR-28771 : correctif pour CQ-4259676
...
* Exception NullPointerException dans TargetContentManager. Correctif pour CQ-4263485
...
* Mise à à jour de Jsafe Jars vers Cryptoj 6.1.3.1 dans le programme d’installation et LCM. NPR-21370
...
* Modification de l’interface utilisateur pour prendre en charge la mise en œuvre de la fonctionnalité CUG dans les ressources AEM.
...
* Modification de l’interface utilisateur pour prendre en charge la mise en œuvre de la fonctionnalité CUG dans les ressources AEM. NPR-19485
* Le téléchargement d’une ressource en tant que nœud enfant direct de soi-même à l’aide de l’IU tactile provoque un problème. La ressource est téléchargée en tant qu’enfant direct de la ressource précédemment sélectionnée. NPR-19736
...
* Impossible de télécharger des fichiers avec des espaces à partir de la page de vérification de licence. NPR-20067 : correctif pour CQ-4216557
* Problèmes de traitement lors du téléchargement de fichiers PSB avec plusieurs calques alpha. NPR-20250 : correctif pour CQ-4220869
...
* Les fichiers compressés contenant des fichiers dont les noms ne sont pas compatibles avec UTF-8 ne sont pas correctement téléchargés. NPR-20420 : correctif pour CQ-4219961
...
* Un lot de courriels d&#39;apache commons `{org.apache.commons/commons-email/1.5}` a été ajouté en remplacement de `{com.day.commons.osgi.wrapper/com.day.commons.osgi.wrapper.commons-email/1.2.0-0002}`.
...
* Les modifications apportées aux propriétés des champ dans plusieurs onglets de la boîte de dialogue de modification des propriétés ne sont parfois pas enregistrées. NPR-20286
...
* Le module externe Richtext ne fonctionne pas pour les barres en ligne et en plein écran dans l’IU tactile multichamp. NPR-20973
...
* Le sélecteur d&#39;utilisateurs de granit prêtes à l&#39;emploi ne charge aucun utilisateur lorsque vous cliquez sur la liste déroulante dans l&#39;étape participant de la boîte de dialogue du modèle de processus. NPR-20236
...
* Il manque la prise en charge de l’expression d’agrégation pour les panneaux répétés. NPR-20861
...
* Gérez les listes d&#39;envoi du flux de travail uniquement dans dix applications. CQ-4230193

#### Formulaires HTML5 {#html-forms-3}

* Le nom d’un objet de formulaire XDP tel qu’un sous-formulaire s’affiche en tant qu’info-bulle lorsqu’il n’est pas défini dans les configurations d’accessibilité. NPR-20523
...
* Installez la dernière bibliothèque CTJPEG2K pour identifier une vulnérabilité de sécurité critique. Cela a un impact sur les modules XMLFM (AEM et IfBA), RM, PDFG. NPR-20625 : NPR-21337.
...
FileAEM Cumulative Fix Pack 6.3.1.1 est une mise à jour importante qui comprend plusieurs correctifs internes et clients depuis la disponibilité générale de AEM 6.3 Service Pack 1 (6.3.1.0) en octobre 2017.
...
* Correctifs de bogues HTL pour AEM 6.3.1. NPR-19161
* Le champ de texte enrichi n’est pas modifiable dans les composants multi-champs. NPR-19604 : correctif pour Granite-16755
* Le service Modèle de courrier électronique Adobe ajoute des balises aux modèles utilisateur personnalisés. NPR-19190
...
* Une exception NullPointerException se produit lorsqu’un fichier est déplacé depuis sa page de propriétés. CQ-4204251
* AEM ne parvient pas à suivre les références ultérieures à un fichier dans la page de propriétés s’il est lié plusieurs fois à un document InDesign. CQ-4204186
...
* Lorsque des fichiers en double sont téléchargés, l’option (supprimer/conserver) est appliquée à tous les fichiers, même s’ils ne sont pas sélectionnés dans la boîte de dialogue des doublons détectés. CQ-4201673
...
* Composants localisés modifiés non restaurés dans leur formulaire d’origine lorsque l’héritage est réappliqué dans LiveCopy. NPR-18172 : correctif pour CQ-4211379
...
* Résolveur de ressources non fermé dans les objets suivants :
   ...
   * `com.day.crx.delite.impl.AuthHttpContext` NPR-17493 : PCP pour GRANITE-17404
...
Les correctifs d’AEM Forms sont fournis par le biais du module complémentaire Forms et d’autres programmes d’installation de patchs fournis avec la version. Pour plus d’informations, voir [AEM Forms Release](aem-forms-releases.md).
...
* Le service PDF Generator ne parvient pas à convertir des images de plus de 100 Mo en documents PDF. Ref# CQ-4208628
...
aem Cumulative Fix Pack 6.3.0.1 est une mise à jour importante qui comprend plusieurs correctifs internes et clients depuis la disponibilité générale d&#39;AEM 6.3 en avril 2017. Les points saillants de l&#39; Cumulative Fix Pack sont les suivants :
...
* Améliorations dans les domaines suivants :
   ...
   * Révision sociale et connexion à Facebook Social
...
* Fournit une option de validation dans Package Manager pour détecter les conflits entre le recouvrement et le CFP
...
Vous pouvez télécharger le package CFP directement à partir de [Software Distribution](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/cumulativefixpack/AEM-CFP-6.3.3.8) ou effectuer les étapes suivantes :
...
>
>Vous pouvez accéder à la documentation de l’option Valider à l’adresse [https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator](https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator).

* AEM 6.3.3.0 est une condition préalable à l’installation du CFP. Consultez la [documentation de mise à niveau](https://docs.adobe.com/docs/fr/aem/6-3/deploy/upgrade.html ) pour obtenir des instructions détaillées sur la mise à niveau d’une installation AEM vers AEM 6.3.
...
* Vous pouvez ajouter une nouvelle journalisation à l’emplacement par défaut [http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog) avec les propriétés suivantes :

   * Niveau de consignation : débogage
   * Additif : faux
   * Fichier journal : logs/activity.log
   * Journaliste : org.apache.jackrabbit.vault.emballage.impl.ActivityLog

Le fichier activité.log sera créé dans   dossier crx -quickstart /logs.

### Installer le Cumulative Fix Pack via Software Distribution {#install-the-cumulative-fix-pack-via-package-share}

Suivez les étapes suivantes pour installer le pack de correctifs cumulés sur une instance 6.3 existante :
...
* Utilisez l&#39;API [HTTP de Package Manager](https://helpx.adobe.com/fr/experience-manager/6-3/sites/administering/using/package-manager.html) - veillez à utiliser `cmd=install&recursive=true` pour installer le package imbriqué.
...
1. Tous les lots OSGI sont PRINCIPAUX ou FRAGMENT dans la console OSGI (Utiliser la console Web : `/system/console/bundles`).

>[!NOTE]
>
>Une fois l’installation du package terminée, un message d’information s’affiche sur les journaux d’erreurs pour indiquer que le package de contenu a été installé avec succès, par exemple &quot;Package de contenu **AEM-6.3.3-Cumulative-Fix-Pack-7** installé avec succès.&quot;

>[!NOTE]
>
>Depuis AEM 6.3.3.1, le niveau de journal dans Jackrabbit FileVault est passé d’INFO à DEBUG pour la plupart des messages. Pour obtenir les journaux d&#39;installation d&#39;un package de contenu installé sur AEM 6.3.3.1 ou version ultérieure, activez le niveau de journal DEBUG pour org.apache.jackrabbit.vault.packagecle.impl.
...
1. Installez le module complémentaire Forms comme décrit dans la section [Installation des modules complémentaires de formulaires AEM](https://helpx.adobe.com/fr/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html).
...
1. Pour installer la mise à jour, exécutez le fichier Designer 6.2.0_&lt;Language>_Cumulative_QF.msp.
...
* Les erreurs et avertissements suivants peuvent survenir lors de l&#39;installation de AEM CFP 6.3.3.x et peuvent être ignorés en toute sécurité :

   * *WARN* [OsgiInstallerImpl] org.apache.jackrabbit.vault.packl.impl.InstallHookProcessorImpl Hook /META-INF/vault/hooks/cloudservices-wfchangeinstallhook-0.0.2-jar-with-dependencies.jar a levé l&#39;exception d&#39;exécution.
   * *ERROR* [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)] Délai d&#39;attente d&#39;exécution de la modification de reg non enregistrée. CQ-4209974.
   ...
   * *ERROR* [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR (org.osgi.framework.ServiceException : La fabrique de services a renvoyé la valeur null. (Composant : com.day.cq.dam.handler.standard.ps.PostScriptHandler))
...
>[!NOTE]
>
>Ce paramètre de configuration s’applique à CFP 6.3.3.2 et versions ultérieures. Il vous demande de mettre à jour les propriétés de délégation de démarrage dans le fichier `sling` .properties.
...
* Ouvrez adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml pour le modifier :
   ...
   * mettez à jour la valeur **sling.bootdelegate.ibm** param-name avec :
...
* Désinstallez le fichier EAR (Enterprise Archive) précédent du serveur d’applications WebSphere et installez le fichier EAR mis à jour en suivant les étapes décrites dans la section 10.2 de [https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf).
* Enregistrez le fichier et redémarrez le serveur. [https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf)

## Paramètres de configuration requis pour NPR-23208 {#configuration-settings-required-for-npr-1}

>[!NOTE]
>
>Ce paramètre de configuration s’applique aux versions 6.3.2.2 et ultérieures. Il vous demande de mettre à jour manuellement les stratégies des Listes de Contrôle d&#39;accès (ACL) via CRX-DE, car les ACL ne sont pas mises à jour via l&#39;installation CFP en raison de la fonction &quot;merge_preserve&quot; acHandling.
...
`1)` Au chemin &quot;/content&quot;\
`a)` Principal : service d&#39;ajustement de référence\
Type : Autoriser\
Privilèges : jcr:read, jcr:modifyProperties\
Restrictions : rep:glob=&quot;/*/jcr:content&quot;\
`b)` Principal : service d&#39;ajustement de référence\
Type : Autoriser\
Privilèges : jcr:read, jcr:modifyProperties\
Restrictions : rep:glob=&quot;/*/jcr:content/*&quot;

`2)` Au chemin d’accès &quot;/content/usergenerate&quot;\
`a)` Principal : service d&#39;ajustement de référence\
Type : Autoriser\
...
La propriété contrôle la profondeur maximale de la sous-arborescence du noeud sous le noeud de page ` /  jcr   :content`, jusqu&#39;à ce que les noeuds présents dans le référentiel soient utilisés pour obtenir les propriétés de la page. Tout nœud situé sous la profondeur spécifiée dans cette propriété est ignoré.

La valeur par défaut est 1. Cette valeur peut être  remplacez par superposer la propriété file constants.js (`/libs/cq/ui/widgets/source/constants.js`) sans commenter la propriété CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL et lui affecter la valeur requise (profondeur maximale sous la propriété / jcr:content de la page, jusqu&#39;à laquelle les données des propriétés de la page sont stockées).

**Si l’utilisateur doit créer plusieurs variantes de pages afin que le nombre de noeuds sous le noeud de la page / jcr:content soit supérieur à 1 000, procédez comme suit pour modifier la configuration :**

* Configuration de la propriété JSON Max results d’Apache Sling
* Get Servlet using `/system/console/  configMgr`
* Définissez sa valeur sur un nombre >1000 (valeur par défaut actuelle) de telle sorte que ce nombre soit supérieur au nombre total de noeuds dans la sous-arborescence / jcr:content jusqu’à la profondeur configurée ci-dessus.

Cela permet à la servlet de GET sling de renvoyer tous les noeuds requis.

## Uber Jar {#uber-jar}

Le Jar Uber pour la version 6.3.3.8 est disponible dans le [référentiel Adobe Public Maven](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.3.3.8/).

Pour utiliser Uber Jar dans un projet Maven, reportez-vous à l&#39;article [Comment utiliser Uber jar](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/ht-projects-maven.html) et incluez la dépendance suivante dans votre POM de projet :
...
>[!MORELIKETHIS]
>
>* [Versions et mises à jour d’AEM](https://helpx.adobe.com/fr/experience-manager/aem-releases-updates.html)
>* [Page des correctifs AEM 6.3](https://helpx.adobe.com/fr/experience-manager/kb/aem63-available-hotfixes.html)
>* [Notes de mise à jour d’AEM 6.3](https://docs.adobe.com/docs/fr/aem/6-3/release-notes.html)
>* [Page de produits AEM ](http://www.adobe.com/fr/solutions/web-experience-management.html)
>* [Documentation d’AEM 6.3](https://docs.adobe.com/content/docs/fr/aem/6-3.html)
