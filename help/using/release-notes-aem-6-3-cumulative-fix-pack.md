...
>[!NOTE]
>
>MongoDB Enterprise 3.6 est pris en charge à partir de AEM version 6.3.3.1.
...
* Le chargement des propriétés de page d’une page modèle échoue et une exception NullPointerException est renvoyée. Le problème est résolu lors de l’ajout de la propriété cq:blueprint (NPR-30901).
...
* Impossible de synchroniser la campagne dans Adobe contenant le composant d’importateur de conception prêt à l’emploi (NPR-30890).
...
* OutputService affiche une réponse incorrecte après l’application d’un correctif pour résoudre les problèmes de HTML des PDF (NPR-31504).
...
* Le traitement des sous-ressources de fichiers de PDF volumineux entraîne une exception OutOfMemoryError (OOME). NPR-29851 : correctif pour CQ-4269574
...
* CoralUI, lorsqu’il est utilisé avec Multifield, stocke le paramètre fileReferenceParameter au niveau du composant plutôt qu’au niveau de plusieurs champs. NPR-29535 : correctif pour CQ-4266129
* Problème lors de la tentative de comparaison de la version de page la plus récente et la plus récente sur AEM 6.3.3.3. NPR-29532 : Correctif pour CQ-4269639
...
* La réduction de l’éditeur de texte enrichi à partir du mode plein écran en mode d’édition de la source entraîne une perte de contenu. NPR-28838 : correctif pour CQ-4260584
...
* Les valeurs sont différentes entre la page de collection et la page de collection à l’aide du filtre . NPR-28642 : correctif pour CQ-4261405
...
* La diffusion vidéo de bureau DMS7 à l’aide de DMComponent utilise le téléchargement progressif pour lire la vidéo en mode de publication et ne diffuse pas de vidéo. NPR-28754 : correctif pour CQ-4263732
...
* La réplication d’AEM vers Brand Portal est bloquée pendant de longues périodes. NPR-28913 : correctif pour CQ-4254932

### Communities {#communities-3}

* Impossible d’ouvrir les messages dont les pièces jointes se trouvent dans le dossier Envoyé et Boîte de réception de Outlook. NPR-28559 : correctif pour CQ-4217072

### Sites {#sites-4}

* Cross-site scripting (XSS) dans SuggestionHandler pour 6.3. NPR-28692 : Correctif pour CQ-4253821
* Le déploiement profond se termine sans contenir toutes les branches dans la Live Copy correspondante. NPR-29175 : correctif pour CQ-4239472
...
* L’ouverture de la variation dans le mode Carte provoque une erreur 500. NPR-28611 : correctif pour CQ-4263571
...
* L’utilisation des informations d’identification Brightedge échoue en raison d’une erreur de connexion. NPR-29167 : correctif pour CQ-4265872
* Problème de compilation dans OfferproxyTandtProvider.java en raison d’une instruction d’importation manquante pour la classe &#39;Resource&#39; : instruction d’importation manquante : importez org.apache.sling.api.resource.Resource. NPR-28772
...
* Option activée permettant de sélectionner des éléments par page sur la page d’affichage Jeu de stratégies.
...
* Un comportement différent est observé lors de la prévisualisation d’un XDP en tant que PDF par rapport au même XDP que HTML dans Designer. NPR-28602 : correctif pour CQ-4260239
...
* Le référentiel intégré (Apache Jackrabbit Oak) a été mis à niveau vers la version 1.6.16.
* Modifications de la pagination de la liste des paramètres de stratégie pour limiter 50 enregistrements par page.
...
* Ajout d’un caractère d’échappement pour la virgule lors d’une recherche.
* Activation de la prise en charge des ressources de synthèse pour la stratégie de contenu.
...
* Activation de la navigation au clavier pour les composants d’activation d’AEM Communities sur Publier. NPR-27739 : correctif pour CQ-4253856
...
* DispatcherFlushRules interrompt la réplication dans la version 6.3.3.1 NPR-28150 : Correctif pour CQ-4261401
...
* Le téléchargement via l’e-mail de partage de lien renvoie un fichier zip vide/corrompu. Correctif pour CQ-4259686
...
* Impossible d’effectuer le rendu des PDF forms sur l’espace de travail HTML5. NPR-28059 : correctif pour CQ-4260373

#### Forms - Document Services {#forms-document-services-2}

* Impossible d’afficher les jeux de politiques au-delà des 1 000 premiers répertoriés dans la vue Jeux de politiques de la console d’administration. NPR-28060, NPR-26047 : correctif pour CQ-4249865
...
* Améliorez les requêtes pour utiliser la fonctionnalité ExpiryNotification . NPR-26999 : correctif pour CQ-4251188
* Migration des règles du schéma de métadonnées vers le schéma de métadonnées de dossier. NPR-27771 : Prise en charge de CQ-4257737, CQ-4257735, CQ-4259822
...
* Le téléchargement via l’e-mail de partage de lien renvoie un fichier zip vide/corrompu. NPR-27997 : correctif pour CQ-4259686
...
* L’héritage de la configuration du Cloud Service rompt lors de la modification d’autres propriétés de page. NPR-27311 : correctif pour CQ-4256785
...
* Lorsque l’éditeur de texte enrichi est associé à plusieurs champs, le TypeError non intercepté : fieldAPI.getName n’est pas une fonction lorsque l’erreur foundation.js se produit. NPR-27146 : Correctif pour CQ-4253155, CQ-4259967
...
* La notification du nombre non lu ne peut pas être définie au-delà de 21. NPR-27496 : correctif pour CQ-4252829
...
* Les relations avec le contenu créé par l’utilisateur ne sont pas supprimées lors de la suppression du contenu créé par l’utilisateur. NPR-27631 : correctif pour CQ-4258706
...
* Correction d’un problème lié à la gestion des données de formulaire pour la valeur par défaut.
...
* Le partage d’un lien de ressource à partir de plusieurs clients ajoute un identifiant de tenant formant une URL non valide. NPR-26482 : correctif pour CQ-4253540
...
* La création d’un grand nombre de relations sur une ressource entraîne une erreur. NPR-26309 : correctif pour CQ-4250708
...
* Rétroportage proactif des correctifs Sites. NPR-26963
* Les balises sont créées deux fois lorsqu’il existe une différence de casse entre le type Titre et Nom. NPR-26877 : correctif pour CQ-4254134
...
* TSDK incorporé avec AEM interrompt avec une mise à niveau vers Handlebars 4 en raison de l’utilisation de modèles incompatibles. NPR-26699 : correctif pour CQ-4248974
...
* Exceptions dans les journaux lorsque le noeud de jeton d’accès est accessible simultanément par plusieurs agents de réplication. NPR-26964 : Correctif pour GRANITE-23276, Granite-23155
...
* AEM fournisseur OAuth n’effectue pas de recherche non sensible à la casse. NPR-26133 : correctif pour GRANITE-22650
...
* La notification &quot;Marquer tout en lecture&quot; rend uniquement les 10 premières comme non lues sans actualisation de la page. NPR-27036 : correctif pour CQ-4254058
* Comportement de pagination des sections de commentaires Communities et de chargement de page. NPR-27030 : correctif pour CQ-4251228
* (Recherche dans le forum) Le bouton Précédent saute la page et revient à forum.html. NPR-26949 : correctif pour CQ-4254804
* La notification non lue n’augmente pas de plus de 21. NPR-26947 : correctif pour CQ-4251251
* (Tâche SearchScheduledPosts) Ajout du commutateur activer/désactiver dans ConfigMgr. NPR-26924 : correctif pour CQ-4250463
* Le chemin d’accès de Tomcat Context(/aempublish) se perd lors du défilement de la page Affectations. NPR-26919 : correctif pour CQ-4254345
* NullPointerException lors de la création d’un groupe et d’un membre. NPR-26778 : correctif pour CQ-4248095
...
* Workspace se bloque par intermittence et affiche les points de départ à plusieurs reprises. NPR-26461 : correctif pour CQ-4253439
...
* Activation des sous-ressources et de la visionneuse de ressources multi-pages dans Brand Portal.
...
* Correctif proactif pour vous protéger contre les promotions de contenu indésirables. NPR-24387 : correctif pour 4244993
...
* La collecte dynamique n’affiche pas les résultats après l’enregistrement si vous utilisez le prédicat de balises dans la facette de recherche. NPR-23401 : correctif pour Granite-21278
...
* Le nom de la balise de plainte JCR doit être automatiquement renseigné en fonction du titre de la balise. NPR-24173 : correctif pour CQ-4199411
...
* La table Foundation est vulnérable aux scripts intersites stockés. NPR-23214 : correctif pour CQ-4240760

### Traduction {#translation-3}

* Propriétés de Live Copy non supprimées lors de la création de copies de langue. NPR-23123 : correctif pour CQ-4230641
...
* Sélectionner RT ne valide pas lorsque l’élément avec une valeur vide est sélectionné. NPR-23405 : correctif pour Granite-21283
...
* Les connexions de suivi client Apache Http et une utilisation élevée du tas entraînent le blocage du serveur AEM. NPR-23906 : correctif pour Granite-21056
...
* La recherche sur les communautés échoue pour peu de mots. NPR-23256 : correctif pour CQ-4240717
* Impossible d’affecter des groupes pour le problème de rôle des gestionnaires de communautés. NPR-23317 : correctif pour CQ-4241233 : CQ-4221399
...
* Les idées supprimées apparaissent comme des liens principaux dans les activités. NPR-23406
* imsmanifest.xml ne peut pas être chargé avec AEM s’exécutant avec la racine de contexte. NPR-23483 : correctif pour CQ-4242193
...
* Les problèmes avec les composants de communautés lorsqu’ils sont accessibles via le Dispatcher et la dynamique Sling incluent l’activation de cette option. NPR-23586 : Correctif pour CQ-4242360, CQ-4241522
* Lors de la recherche d’un terme de recherche qui génère de nombreux résultats par le biais de la recherche, puis qui entre dans un nouveau terme, la pagination n’est pas réinitialisée. NPR-23739 : correctif pour CQ-4222593
...
* Aucun détail sur les utilisateurs de l’indicateur et les raisons n’est affiché à l’aide de la configuration DSRP. NPR-23973 : correctif pour CQ-4243205
* Les raisons de l’indicateur des utilisateurs non marqués restent visibles/NPR-23974 : Correctif pour CQ-4243822
...
* Correctifs de plusieurs problèmes Adobe Social. NPR-24242 : correctif pour CQ-4245054, CQ-4245120, CQ-4245296
...
* L’opération LiveCopy n’ignore pas les noeuds enfants des composants supprimés. NPR-23058 : correctif pour CQ-4211662
...
* Le service de capture de papier se bloque pendant le traitement simultané des fichiers de TIFF ( taille d’environ 50 Ko ). NPR-23556
...
* Ajoutez la prise en charge de PDF/UA aux formulaires XML générés via Designer et Output Service. NPR-23132
...
* Publication sur Brand Portal asynchrone pour tous les types de ressources.
...
* Plusieurs paramètres empilés en raison de requêtes longues (plus de 6) rendent AEM léthargique. NPR-22134 : correctif pour CQ-4233904
* Impossible de modifier les autorisations sur les noeuds dont le nom contient deux points. NPR-22136 : correctif pour CQ-4236221
...
* Rendre TreeNode de secours à l’attribut name lorsque le texte est vide. NPR-22146 : Correctif pour CQ-4234724 / CQ-4236300
...
* Le champ Dialog FileUpload obligatoire entraîne des problèmes lors de l’envoi de la boîte de dialogue. NPR-22464 : correctif pour CQ-4222192
* Le déplacement sans autorisations de réplication lance une demande de workflow d’activation si la page déplacée ou ses référents ne peuvent pas être activés. NPR-22467 : correctif pour CQ-4211765
...
* (StyleSystem) Un saut s’applique à la position de l’écran lors de la sélection du style dans le pop-up. NPR-23183 : correctif pour CQ-4238867
...
* Problèmes de performances avec org.apache.sling.i18n session partagée/résolveur de ressources. NPR-23390 : Correctif pour Sling-7543

### Intégration {#integration-5}

* ResourceResolver non fermé dans com.day.cq.analytics.sitecatalyst. NPR-22323 : correctif pour CQ-4236515
...
* Lors de la fourniture d’un fichier at.js ou mbox.js personnalisé, le script d’inclusion est écrit sur la page en tant que texte au lieu de balises de HTML. NPR-22441 : correctif pour CQ-4203691
...
* Le codage d’entrée SOAP UTF-8 dans Adaptive Forms renvoie des erreurs avec une page altérée. NPR-20105 : correctif pour CQ-4222669
...
* Les Forms qui sont modifiées après la publication de la page ne sont pas republiées lors de la publication du site. NPR-23013 : correctif pour CQ-4236566
...
* Empêcher les navigateurs de stocker les informations d’identification saisies dans les formulaires de HTML. NPR-21762 : correctif pour CQ-4206543
...
* Objets de chemin, par exemple : Les bordures, les soulignements et les hyperliens ne sont pas balisés lors de l’enregistrement en tant que PDF à partir de Designer. NPR-23136
* L’objet Image ne comporte pas de cadre de sélection lorsqu’il est enregistré en tant que PDF à partir de Designer. NPR-23134
* les liens hypertexte intégrés définis dans Designer ne sont pas balisés ni ne comportent de texte de remplacement lors de l’enregistrement en tant que PDF à partir de Designer. NPR-23133
...
* Exception rencontrée lors de la tentative de signature/certification numérique d’un document de PDF via HSM. NPR-21154 : correctif pour CQ-4226978
...
* Résolution de plusieurs problèmes de gestion des dossiers avec les API Assets. NPR-20569
...
* Problèmes d’autorisation d’administrateur : Impossible de supprimer les membres du groupe d’utilisateurs fermé des propriétés de la page. NPR-20631
* Le nom du processus saisi doit être disponible dans la zone Notification lors de la publication de la page via Gérer la publication. NPR-20046 : correctif pour CQ-4221586
...
* Les notifications par courrier électronique ne sont déclenchées pour aucune des activités liées au modérateur sur les composants publiés. NPR-20122
* La pagination vierge sans résultat s’affiche pour les utilisateurs anonymes de la communauté. NPR-20136 : correctif pour CQ-4220738
* Configurez le déclencheur d’alerte lorsqu’un contenu généré par l’utilisateur est détecté comme indésirable ou lorsqu’un utilisateur publie sur un site prémodéré. NPR-20274 : correctif pour CQ-96850
...
* CommunityUserOperationExtension s’exécute dans une exception de diffusion de classe. NPR-20532 : correctif pour CQ-4224385
...
* requestAttributes n’est pas non défini pour les autres instructions data-sly-resource. NPR-20639 : correctif pour 4226206
...
* L’installation d’AEM 6.3.0.1 supprime le KeyStore de l’utilisateur dam-update-service. NPR-20018
* Dans certains déploiements personnalisés, les utilisateurs qui tentent de sélectionner une personne désignée dans le module addTask prennent plus de temps pour remplir la liste dans le sélecteur d’utilisateur. NPR-20283 : correctif pour CQ-4224193

### Interface utilisateur {#user-interface-3}

* Colorfield est défini sur &quot;Toujours requis&quot; malgré les attributs dans la boîte de dialogue. NPR-19702
* La barre de défilement ne s’affiche pas pour le composant à plusieurs champs en plein écran dans Internet Explorer 11. NPR-20261 : correctif pour CQ-4219782
...
* Impossible de définir la valeur du composant switch entre true/false. NPR-21111
...
* Dynamic Viewer charge le rendu vidéo de qualité inférieure pour tester l’expérience de lecture vidéo en mode création. NPR-19004
...
* Les valeurs de liste déroulante de champs multicomposites ne sont pas rérenseignées en raison de la modification du code dans Sidekick.js après la mise à niveau de l’instance vers AEM 6.1SP2-CFP3. NPR-19450 : HF pour CQ-4194771
...
* Le courrier est déclenché lors de la publication de contenu avec des sites prémodérés. NPR-20008
* Les notifications par courrier électronique ne fonctionnent pour aucune des activités liées au modérateur sur les composants publiés. NPR-19767 : HF pour CQ-4218060
...
* La modification en masse des propriétés de page n’est pas correctement utilisable, car tous les onglets et champs sont actuellement disponibles pour l’édition en bloc. NPR-19451
* Les propriétés prêtes à l’emploi et les propriétés d’alignement des images n’apparaissent pas lors de la modification d’une image dans l’IU classique. NPR-19488 : HF pour CQ-4217914

### Interface utilisateur {#user-interface-4}

* Lors de l’utilisation du navigateur Google Chrome, les utilisateurs ne peuvent pas charger la liste complète des pages/ressources en mode Colonnes dans l’interface utilisateur tactile. NPR-19566 : correctif pour CQ-4214248

### Brand Portal {#brand-portal-1}

* Impossible de publier des ressources à partir d’AEM avec commentaires et annotations. NPR-19590 : correctif pour CQ-4218386
...
* Les Forms adaptatives n’autorisent pas PDF en tant que pièce jointe dans Safari. NPR-19625
* RestoreGuideState remplace customcontextproperty map. CQ-4222877
...
* Les caractères UTF-8 en sortie du service SOAP ne sont pas correctement copiés dans les champs Adaptive Forms pour les langues autres que l’anglais. NPR-20238 : NPR-20103

#### Correspondence Management {#correspondence-management-1}

* Copier le contenu du fichier Word perd sa couleur et sa police de contenu dans l’éditeur de texte. NPR-19521
...
* Le champ de texte multiligne (TextArea) ne conserve pas de nouveau caractère de ligne ni de saut de ligne après l’envoi de données dans l’espace de travail HTML. NPR-20085

#### Rapports de workflow {#process-reporting}

* Les rapports de processus ne récupèrent pas correctement les données en raison d’une exception de pointeur de valeur NULL. NPR-19759
...
* Lorsque l’on arrête le serveur AEM Forms qui exécutant en configuration de grappe sur l’application JBoss, le serveur d’applications est déconnecté de la base de données. Cela peut entraîner des problèmes de corruption des données. NPR-19724
...
* Configuration simplifiée pour les services de contenu dynamique
...
* Amélioration de l’utilisation des pages riches en contenu en mode Colonnes
* Amélioration de la conformité à la convention de dénomination des pages pour les pages avec des titres longs
...
* Exigence de la vérification des modifications des autorisations utilisateur dans AEM. NPR-17061
* Connexions persistantes aux Cloud Services DM provoquant des exceptions &quot;Trop de fichiers ouverts&quot;. CQ-4211407

### Ressources {#assets-12}

* Problèmes d’utilisation liés à la configuration des services de contenu dynamique à l’aide de différentes options. NPR-18200 : correctif pour CQ-4201557
...
* Les utilisateurs disposant d’autorisations en lecture seule sur l’application ne peuvent pas prévisualiser le contenu à partir de la gestion de contenu AEM Mobile. NPR-17486 : CFP pour CQ-4209690
...
* Problèmes liés à la navigation dans une page fortement chargée de contenu en mode Colonnes dans l’interface utilisateur tactile. NPR-17799 : correctif pour CQ-4199611
...
* Problèmes de personnalisation des campagnes et des expériences associées via l’interface utilisateur classique. NPR-18127 : correctif pour CQ-4211559
...
* Dans un workflow non transitoire, les modifications apportées à l’historique du processus et aux métadonnées avant une étape de processus externe ne sont pas conservées. NPR-17848 : correctif pour GRANITE-17757
...
* La restructuration des capsules dans le module Projets la rend moins configurable. CQ-4209859
...
* Correctifs dans les modules de texte de Correspondence Management, les aperçus de lettres et le lancement par programmation de l’interface utilisateur de création de correspondance.
...
* Lorsque l’envoi par HTML a utilisé la méthode POST, le lancement de l’interface utilisateur de création de correspondance échoue. NPR-18202
...
* Lors de la validation d’un document PDF pour conformité à la norme PDF/A-1b, AEM Forms renvoie une erreur de validation : PDFA_CONTENT_003_DEVICE_DEPENDENT_COLOR_USED. Le document du PDF ne renvoie pas l’erreur lorsqu’il est validé avec Adobe Preflight et des logiciels tiers. NPR-18011
* Lors de la validation de documents PDF pour la conformité à la norme PDF/A-1b, AEM Forms renvoie une erreur de validation : le champ de formulaire a plusieurs apparences. Les documents du PDF sont conformes à la norme PDF/A-1b. NPR-18013

**Dossier de contrôle**

* Lors de la sélection pour traiter les fichiers à l’aide d’un workflow, lors de la création d’un dossier de contrôle, la liste déroulante de sélection de modèle de workflow apparaît tronquée. NPR-17566
...
* Lorsqu’une nouvelle stratégie est créée et qu’une stratégie portant le même nom que celle qui existe, une erreur de serveur interne se produit. NPR-18247
...
* Les erreurs et avertissements suivants peuvent survenir lors de l’installation du CFP AEM 6.3.3.x et peuvent être ignorés en toute sécurité :
   ...
   * &#42;ERROR&#42; [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)] Délai d’attente avant que la modification du reg ne soit terminée sans enregistrement. CQ-4209974.
   ...
   * org.apache.sling.servlets.resolver.internal.SlingServletResolver Erreur d’origine nulle
   * org.apache.sling.engine.impl.DefaultErrorHandler Échec du gestionnaire d’erreurs : java.io.IOException
   * &#42;ERROR&#42; [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR (org.osgi.framework.ServiceException : La fabrique de services a renvoyé une valeur nulle. (Composant : com.day.cq.dam.handler.standard.ps.PostScriptHandler))

**Brand Portal**

* À compter de la version 6.3.1.2, le bouton Publier sur Brand Portal pour les collections dynamiques a été supprimé.

**Interface utilisateur**

>[!NOTE]
>
>Si l’un de ces deux problèmes vous concerne, contactez [Assistance clientèle AEM](https://helpx.adobe.com/fr/marketing-cloud/contact-support.html).
...
>[!NOTE]
>
>Ce paramètre de configuration s’applique aux versions 6.3.2.2 et ultérieures. Il vous demande de mettre à jour manuellement les politiques de listes de contrôle d’accès (ACL) via CRX-DE, car les listes de contrôle d’accès ne sont pas mises à jour via l’installation du CFP en raison de la variable « merge_preserve » acHandling.
...
La propriété contrôle la profondeur maximale de la sous-arborescence du nœud sous le nœud de page ` /  jcr   :content`, jusqu’à ce que les nœuds présents dans le référentiel soient utilisés pour obtenir les propriétés de la page. Tout noeud situé sous la profondeur spécifiée dans cette propriété est ignoré.
