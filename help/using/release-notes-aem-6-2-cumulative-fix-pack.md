...
description: En savoir plus sur les notes de mise à jour du pack de correctifs cumulatif AEM 6.2.
...
# Notes de mise à jour d’AEM 6.2 Cumulative Fix Pack{#release-notes-aem-cumulative-fix-pack}
...
| **Prérequis** | [AEM 6.2 Service Pack 1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) |
...
Pour plus d’informations sur le CFP et sur d’autres types de versions, consultez le [Véhicule de version de maintenance](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).

## À propos de cette version {#about-the-release}

Le pack de correctifs cumulés d’AEM 6.2 SP1-CFP20, dernier pack de correctifs cumulés pour AEM 6.2, est une mise à jour importante comportant des correctifs client importants publiés depuis la disponibilité générale d’[AEM 6.2 SP1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).

>[!CAUTION]
>
>L’application d’un CFP sans valider la compatibilité entre les Feature Packs installés peut entraîner une défaillance du système ou la perte de configurations personnalisées, ce qui peut nécessiter une restauration à partir de la sauvegarde pour être résolu.

>[!NOTE]
>
>* Un nouveau bundle Sling `discovery- api` Johnzon 1.0.0 est inclus avec le pack de correctifs cumulés d’AEM 6.2 SP1-CFP10. En outre, un utilisateur ou une utilisatrice de service sling-discovery est ajouté avec les autorisations de lecture et d’écriture pour le nœud */var/discovery* dans le référentiel CRX.
>
>* Le bundle d’e-mail d’Apache Commons **org.apache.commons/commons-email/1.5** a été ajouté pour remplacer **com.day.commons.osgi.wrapper/com.day.commons.osgi.wrapper.commons-email/1.2.0-0002**.
>
>* Adobe recommande de déployer le pack de correctifs cumulatifs via le dossier d’installation pour les clientes et les clients possédant de nombreux utilisateurs et utilisatrices sur leur instance AEM.
>
...
* Le framework de protection CSRF ne fonctionne pas avec AEM formulaires de base. NPR-28612 : correctif pour GRANITE-22231
...
>[!NOTE]
>
>Pour les clients AEM Forms, il est essentiel d’installer le module complémentaire AEM Forms après l’installation de tout Service Pack, pack de correctifs cumulés ou pack de fonctionnalités AEM.
...
* Lors de l’aperçu de la lettre en tant que PDF, le champ de texte placé sur la page principale ne respecte pas la valeur saisie à partir de l’onglet des données ni la liaison de données spécifiée. NPR-29239 : correctif pour CQ-4266856.
...
Le pack de correctifs cumulés d’AEM 6.2 SP1-CFP19 est une mise à jour importante comportant des correctifs client importants publiés depuis la disponibilité générale d’[AEM 6.2 SP1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).

Les principaux aspects de ce pack de correctifs cumulés sont les suivants :

* Ajout de la prise en charge de l’API MS® Translator v3.0 pour AEM 6.2.
* Ajout d’un message dans le journal après l’installation réussie du package pour tous les SP, CFP et HF.

### Ressources {#assets}

* Vous ne pouvez pas renommer un dossier DAM si l’autorisation Modifier l’ACL est manquante. NPR-27555 : correctif pour CQ-104652
* L’outil d’éditeur de paramètres d’image prédéfinis n’est pas réactif dans la version 6.2.1 CFP 17 et versions ultérieures. NPR-28147 : correctif pour CQ-4261041
...
* Ajout de la prise en charge du service MS® Translator dans AEM 6.3 après la mise à niveau de MS® Translator vers l’API v3.0. NPR-28123 : correctif pour CQ-4259096.
...
* Le dictionnaire n’est pas invalidé pour les bundles de ressources qui utilisent `sling:basename`. NPR-27624
...
* Le module externe de package Granite sélectionne la dépendance d’une version inférieure de org.apache.sling.i18n. Correctif pour CQ-4263245
...
Le pack de correctifs cumulés d’AEM 6.2 SP1-CFP18 est une mise à jour importante comportant des correctifs client importants publiés depuis la disponibilité générale d’[AEM 6.2 SP1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* Ajout de la prise en charge de la redirection au composant de page principal.

### Ressources {#assets-1}

* Les processus `Camera RAW` se bloquent pendant les périodes d’ingestions massives, ce qui finit par bloquer tous les traitements du workflow. NPR-26990 : correctif pour NPR-23860
* La fonctionnalité de téléchargement utilise AEM Assets au moyen d’un servlet de téléchargement de ressources qui permet aux utilisateurs anonymes de télécharger toutes les ressources. NPR-27054, correctif pour CQ-4254732
...
* En raison d’un comportement incorrect de la classe ConfigPostProcessor, la suspension de l’image parente supprime le type MixIn `cq:LiveRelationship` de la page enfant. NPR-26745 : correctif pour CQ-4254163
...
* En raison du tiret cadratin spécial, l’éditeur rencontre un problème lors du vidage du cache. NPR-27199 : correctif pour CQ-4242790
...
* Aucun nouveau correctif dans le module complémentaire AEM Forms.
...
Le pack de correctifs cumulés d’AEM 6.2 SP1-CFP17 est une mise à jour importante comportant des correctifs client importants publiés depuis la disponibilité générale d’[AEM 6.2 SP1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* Modifications apportées à la vue Audiences afin de prendre en charge la structure de dossiers pour l’implémentation de plusieurs clients.
...
* (Vérificateur de liens externes) Les liens https non valides s’affichent comme des liens valides. NPR-25541 : correctif pour CQ-4201333
* (Interface d’utilisation classique) Lors de la création d’une page indépendante sous une Live Copy, la page est créée en tant que Live Copy. NPR-25610 : correctif pour CQ-4249801
...
* La barre d’outils de l’éditeur de texte enrichi de l’éditeur de texte enrichi couvre la liste sélectionnée. NPR-25165 : correctif pour CQ-4248948
* Migrer ContextHub vers jQuery 3. NPR-25059 : correctif pour Granite-19902
* Pour un composant Parsys imbriqué, c’est toujours la première conception satisfaisante (dont le chemin est le moins imbriqué) qui est appliquée à partir de plusieurs composants disponibles. Pour plus d’informations, voir [Résolution du chemin de conception](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-25250 : correctif pour CQ-4246276

### Intégration {#integration-3}

* En utilisant l’intégration cible prête à l’emploi, le ciblage d’un composant effectue le rendu de la page entière au lieu d’un composant ciblé vide. NPR-25273 : correctif pour CQ-4248003
* Après une rupture de l’héritage en mode de ciblage, le composant s’affiche toujours comme ciblé avec l’héritage non rompu en mode d’édition. NPR-25324 : correctif pour CQ-4248162
...
* L’affichage est vide lors de l’utilisation de l’option d’opt-out. NPR-25295 : correctif pour CQ-4246792
* Les expériences supprimées de l’environnement de création ne sont jamais supprimées du site publié lors de l’activation de la page. NPR-24869 : correctif pour CQ-4247832

### DAM - Client DM {#dam-dm-client}

* (Chrome, Firefox) VideoPlayer ignore les clics de souris effectués sur les appareils tactiles. Correctif pour CQ-4247370
...
* Le lot cq-social-as-provider ne commence pas avec les versions 3.x du pilote Mongo. NPR-26271 : correctif pour CQ-4252710
...
* La variable est renseignée en tant que type de données Liste, le sous-type est chaîne, mais une erreur &quot;Impossible de contraindre l’objet&quot; se produit. NPR-26194 : correctif pour CQ-4252287
...
Le pack de correctifs cumulés d’AEM 6.2 SP1-CFP16 est une mise à jour importante comportant des correctifs client importants publiés depuis la disponibilité générale d’[AEM 6.2 SP1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* LineChecker ne parvient pas à externaliser les liens lorsqu’une requête contient des paramètres de requête non valides. NPR-24127 : correctif pour CQ-4241893

### MSM {#msm}

* Correctifs proactifs pour les attaques par scripts de site à site (XSS). NPR-21893 : correctif pour CQ-4223385
* MSM LiveRelationship : RolloutConfig effectif est incorrect si BlueprintConfig est sur la racine. NPR-23999 : correctif pour CQ-4243000

### Sites {#sites-3}

* Pour créer une expérience dans une zone Live Copy, l’héritage devant être rompu en premier doit être configuré. NPR-24995, correctif pour CQ-4248209
...
* L’action Publish sur la page verrouillée rompt la création. NPR-23970 : correctif pour CQ-4243203
...
* La création du lancement échoue lors du lancement du workflow &quot;Demander le lancement&quot; sur un projet. NPR-24255 : correctif pour CQ-4245030
...
* Le bouton Émulateur d’appareil disparaît du mode de modification et de prévisualisation après l’application de CFP14. NPR-24566 : correctif pour CQ-4247060
* (IU classique) Les balises complètes s’affichent comme vides une fois créées dans la boîte de dialogue. NPR-24688, correctif pour CQ-4246407
* Impossible de créer une version lors de la première tentative. NPR-24774 : correctif pour CQ-4232176
...
* Une utilisation élevée du processeur est observée en raison de nombreuses requêtes dans la fonctionnalité de recherche d’administration. NPR-23588 : correctif pour Granite-21286
...
* Lancement de champs multiples NPE lorsqu’aucun FormData n’est présent pour la requête. NPR-24513 : correctif pour Granite-21055
...
* Lorsqu’une lettre est prévisualisée via un aperçu par défaut, le mappage des fragments de mise en page ne s’affiche pas correctement, alors qu’il s’affiche correctement en cliquant sur Aperçu. NPR-22993 : correctif pour CQ-4237745
...
Le pack de correctifs cumulés d’AEM 6.2 SP1-CFP15 est une mise à jour importante comportant des correctifs client importants publiés depuis la disponibilité générale d’[AEM 6.2 SP1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).

Les principaux aspects de ce pack de correctifs cumulés sont les suivants :

* Correctifs de sécurité proactifs dans la table Foundation pour maintenir la cohérence de la conception.
...
* Mettez à jour le dernier fichier adobe-reader-extensions-dsc.jar pour les correctifs dans l’extension Reader.
* Correction du hook de validation afin de tenir compte des éléments `:invalid` pour l’entrée de numéro de rappel.

### Ressources {#assets-4}

* Les données EmbedXMP sont toujours définies sur &quot;active&quot; pour le processus de génération de pyramide de TIFF. NPR-22776 : correctif pour CQ-4234498
...
* (Dynamic Media) Lorsque vous cochez la case Rendus dynamiques , le fichier ZIP téléchargé génère l’image de TIFF d’origine avec un fichier à zéro octet. NPR-22410 : correctif pour CQ-4198471
...
* En mode Ciblage, les auteurs et autrices peuvent modifier un composant hérité du plan directeur sans annuler l’héritage. NPR-22751 : correctif pour CQ-4237907
* La variable path n’est pas correctement codée, ce qui entraîne un script intersite non persistant (XSS). NPR-22851
...
* L’enregistrement du segment avec une valeur énorme (10000000000) définit l’amplification sur 0, provoquant un message d’erreur. NPR-22524 : correctif pour CQ-4237006
* Impossible de cliquer sur Ajouter un élément dans le composant multichamp. NPR-22552 : correctif pour CQ-4237404
* La barre de défilement horizontale n’est pas visible lorsqu’un segment a un titre long. NPR-22615 : correctif pour CQ-4237001
...
* (Firefox) La case à cocher correspondant au personnage sélectionné n’est pas la bonne. NPR-23345
...
* Lorsqu’une lettre est prévisualisée par le biais d’un aperçu par défaut, le mappage des fragments de mise en page ne s’affiche pas correctement, mais il s’affiche correctement lorsque vous cliquez sur le bouton Aperçu . NPR-23335 : correctif pour CQ-4237745
...
* Vulnérabilité aux scripts intersites (XSS) stockée dans l’onglet Remarques du Gestionnaire de Forms. NPR-27157 : correctif pour CQ-4255556

#### Service Encryption {#encryption-service}

* (OSGI) [JEE] État de signature incorrect pour le PDF avec pièce jointe lors de la vérification à l’aide de &quot;Valider le processus du PDF&quot;. NPR-23269, NPR-23737
...
* Reconfiguration du traitement de notification de l’expiration de mots de passe pour les ressources déjà expirées.
...
* Les utilisateurs ne peuvent pas télécharger de ressources avec une clause de non-responsabilité et des noms de fichier longs. NPR-22163 : correctif pour CQ-4235274
* Un guillemet simple empêche la mise à jour des métadonnées dans l’affichage en bloc et l’interface utilisateur est rompue lorsque vous ouvrez les propriétés d’une ressource à l’aide des actions de la barre d’outils rapide. NPR-22317, NPR-22353 : correctif pour CQ-4236990, CQ-4236469
* La tâche Notification d’expiration de ressource ne désactive pas les ressources expirées. NPR-22346 : correctif pour CQ-4237188
...
* (IU tactile) La balise déplacée apparaît aux anciens et nouveaux emplacements dans les propriétés de la page. NPR-21921, correctif pour CQ-4238598
...
* (IU tactile) Impossible d’afficher plus de 40 balises sous un espace de noms. NPR-22290 : correctif pour CQ-99114
* Problèmes de flux RSS, port -1 à AEM 6.2 NPR-22158 : correctif pour CQ-4233339
* (IE) Lors de la première création d’un caractère dans le champ de texte enrichi, un espace de fin est ajouté au caractère. NPR-22443 : correctif pour CQ-4235343
* Lorsque vous tentez de faire correspondre le nom du module, l’objet Use Java™ gèle SightlyJavaCompilerService en raison d’un caractère d’espace à la fin de la déclaration du module. NPR-22557 : correctif pour Granite-20836
...
* (Digital Publishing Suite) La date de publication et la date de couverture sont des champs obligatoires pour les folios avant qu’ils ne soient téléchargés dans DPS. NPR-22484

### Commerce {#commerce}

* Plusieurs vulnérabilités de script de site à site (XSS) dans l’assistant de création de catalogue de commerce. NPR-22344 : correctif pour CQ-4237017
...
* Le moteur Target (mbox.js, at.js) n’utilise pas d’URL gérées et utilise des URL contenant des deux-points qui peuvent échouer avec certains déploiements. NPR-22366 : correctif pour CQ-4237854
...
* (HTML Workspace) Lorsqu’un utilisateur demande une tâche, le nombre de files d’attente est actualisé pour cet utilisateur spécifique uniquement. En d’autres termes, à moins que la page ne soit actualisée. NPR-19778 : correctif pour CQ-4233665
...
* Activation de la configuration de champ Paramètre statique dans les paramètres des composants Target lors de l’utilisation d’AT.js comme bibliothèque cliente.
* Correctifs de la fonctionnalité d’affichage/masquage des composants déroulants.
...
* (Safari) Les utilisateurs ne peuvent pas afficher toutes les ressources dans le panneau de l’outil de recherche de ressources. NPR-21981 : correctif pour CQ-4213720
* Timewarp renvoie une erreur &quot;RecursionTooDeepException&quot; avec une page altérée et aucune nouvelle version n’est créée même si la date est modifiée. NPR-21707 : correctif pour CQ-4199536
* Lors du chargement d’une page dans l’éditeur, le WorkflowStatusprovider (pageinfo.json) est chargé trois fois, ce qui ralentit l’exécution de l’instance AEM. NPR-21778 : correctif pour CQ-59232
...
* Les utilisateurs ne peuvent pas voir les valeurs de paramètre statique de test dans l’appel de mbox qui s’affichent lors du test avec AT.js comme bibliothèque cliente dans la configuration cloud. NPR-21930 : correctif pour CQ-4234520
...
* Les utilisateurs ne peuvent pas se synchroniser avec la synchronisation des utilisateurs à l’aide de la distribution Sling. NPR-21911 : correctif pour Granite-20404
...
Note : ce correctif nécessite Oak CFP 1.4.20 ou version ultérieure.

### Traduction {#translation-3}

* La propriété jcr est absente pour l’ID utilisateur de l’initiateur lors de la création d’un projet de traduction. NPR-21715 : correctif pour CQ-4230713
...
* (iOS11) Lorsque vous cliquez sur une pièce jointe, le fichier correspondant ouvre la caméra au lieu de l’explorateur de fichiers de l’appareil. NPR-21926 : correctif pour CQ-4214348
* Le titre manquant dans l’interface d’utilisation de création de thème provoque une exception et un échec du rendu de la boîte de dialogue. Correctif pour CQ-4236143
...
* Le fichier PDF généré à l’aide de `6.2.0-ASM-1017-003` est endommagé. NPR-21427 : correctif pour CQ-4228046
...
* Trop de caractères dans OmniSearch peut provoquer le blocage du serveur AEM. NPR-21083 : correctif pour CQ-4223602
...
* Lors de l’ajout d’une structure Target, le mode Ciblage n’est pas disponible dans la liste Mode sélectionné de l’éditeur. NPR-21047
...
* Le processus de réinitialisation de la Live Copy est arrêté après la suppression de la première page locale dans le LC. NPR-21276 : correctif pour CQ-4229743

### Plateforme {#platform-5}

* Les bibliothèques de balises personnalisées qui référencent des balises implémentées en tant que script sont introuvables après la mise à niveau vers AEM 6.3. NPR-20149 : correctif pour Granite-18433

### Traduction {#translation-4}

* Les workflows de traduction échouent avec des codes lang_country de plus de deux caractères. NPR-21088 : correctif pour CQ-4197439
* N’envoyez pas à nouveau une page Ressource à un projet de traduction tant que celui-ci n’est pas terminé. NPR-21219 : correctif pour CQ-4209908

### Interface utilisateur {#user-interface-3}

* Le composant Sélectionner ne supprime pas la propriété cible lors de l’envoi du formulaire. NPR-21163 : correctif pour GRANITE-14125
* HTTP.encodePathOfURI développe double code le signe plus &quot;+&quot;. NPR-21417 : correctif pour GRANITE-16340

### Vulnérabilité {#vulnerability-4}

* Cross-site scripting (XSS) dans l’éditeur de métadonnées de gestion des actifs numériques. NPR-21434 : correctif pour CQ-83472
* Plusieurs fichiers de SWF sont vulnérables aux scripts de site à site (XSS). NPR-20612 : correctif pour CQ-4213297
...
* Le bouton d’envoi Aperçu de la lettre est désactivé après l’installation d’AEM 6.2 SP1-CFP9. NPR-21547
* Lorsque vous sélectionnez le type de liaison Ressource, la fenêtre Sélecteur de ressources ne s’ouvre pas dans l’assistant Modifier la liaison de données de lettre. NPR-21164 : correctif pour CQ-4194567
...
* La mise à niveau vers la dernière mise à jour Java™ 8 131 renvoie une exception : &quot;Le fournisseur JsafeJCE est désactivé, une vérification d’intégrité requise FIPS 140 a échoué.&quot; NPR-21355

**Note :** cette erreur nécessite davantage de paramètres. Voir [Dernière mise à jour de Java™ 8](#latest-java-update-throws-an-exception-npr).

* Jsafe Jars mis à jour vers CryptoJ 6.1.3.1 dans Core, Encryption, Signature et Document Security. NPR-21360, NPR-21361, NPR-21356, NPR-21358

#### Installation de LCM {#install-lcm-1}

* Mise à jour des fichiers JAR Jsafe vers CryptoJ 6.1.3.1 dans le programme d’installation et LCM. NPR-21362

#### Service PDFG {#pdfg-service-2}

* Mise à jour des fichiers JAR Jsafe vers CryptoJ 6.1.3.1 dans PDFG. NPR-21359
...
* Jsafe Jars mis à jour vers CryptoJ 6.1.3.1 dans RE. NPR-21357
...
* Mise à jour de cq-msm-core pour une synchronisation Livecopyindex efficace.
* Amélioration de l’efficacité des modifications pour les fragments de contenu.
* Il fournit une option de validation dans le gestionnaire de modules pour la détection des autorisations ACL.
...
* Correctifs dans le composant Sightly et les LiveCopies.
...
* Les utilisateurs ne peuvent pas télécharger de ressources avec une clause de non-responsabilité et des noms de fichier longs. NPR-20255 : correctif pour CQ-4222139
...
* La liaison d’un fichier de ressource codé à l’aide d’un module externe de lien dans l’éditeur de texte enrichi renvoie un lien vide. NPR-20626, NPR-21059 : correctif pour CQ-4223011
* L’éditeur de métadonnées de fragment de contenu empêche les auteurs de contenu d’enregistrer les modifications dans les fragments de contenu. NPR-20641 : correctif pour CQ-4224755
...
* Le composant d’image Sightly &quot;link to&quot; génère une erreur 403 en raison de l’extension .html manquante. NPR-20823 : correctif pour CQ-4195909
...
* Effectuez la mise à niveau vers Apache Commons Email 1.5, qui présente une API simplifiée pour l’envoi d’emails. NPR-20509 : correctif pour Granite-18240
...
* Problèmes d’utilisation avec le composant de griffonnage sur les appareils iOS 10 et 11. NPR-21092
...
* Une exception est générée avec le nom `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` lors de l’appel du service OMPFSubmission/PDFA/PDFtoPDFA. Cette exception empêche le processus de vérification de signature de courte durée de s’achever jusqu’au redémarrage du serveur. NPR-20792
...
* Le processus de préparation des données appelle plusieurs fois lorsqu’un formulaire mobile est rendu avec des paramètres dataref. NPR-19801 : correctif pour CQ-4230427 : CQ-4230400
...
* Ajout de la fonction utilitaire onDialogLoaded pour les tests.
* Ajout de tests et configurations de l’unité front-end à ClientLibraryProxyServlet.
...
* Les ressources miniatures comportant des caractères spéciaux dans le titre affichent un objet Java™ dans l’attribut alt de NPR-20347 : correctif pour CQ-4223620
...
* Les dictionnaires d’internationalisation fonctionnent si le serveur est redémarré dans org.apache.sling.i18n 2.5.6. NPR-20525 : correctif pour Granite - 19490
* Aucun vidage de thread n’est généré en fonction de la période du planificateur avec la configuration par défaut du collecteur de vidage de thread (démarrage AEM par défaut). NPR-20288 : correctif pour GRANITE-19488/GRANITE-12741 CQ-90647.

### Sites {#sites-9}

* Si le filtre de date de modification est modifié après l’ouverture de la recherche enregistrée, il n’y a aucun effet sur les résultats. De plus, les résultats affichés sont identiques à la valeur enregistrée précédente du filtre de date modifiée. NPR-19739 : correctif pour CQ-4219425
...
* (IU tactile) La suppression des lancements par le biais de la console d’IU classique rend toutes les pages non modifiables. NPR-20520 : correctif pour CQ-4225074
...
* Lors d’un accès à l’aide d’une session anonyme, ClientLibraryProxyServlet ne transmet pas de requêtes proxy aux bibliothèques clientes sur l’instance publiée et renvoie une erreur HTTP 404 introuvable. NPR-20195 : correctif pour Granite-14409

### Intégration {#integration-10}

* La sélection du moteur cible comme Adobe Target empêche le chargement du composant et renvoie une erreur dans le journal du serveur. NPR-20058 : correctif pour CQ-88071, CQ-109698, CQ-4201600

### Commerce {#commerce-1}

* Aucun message contextuel de confirmation ou de redirection ne s’affiche lors de la création de produits à partir de la même page. NPR-20257 : correctif pour CQ-4223414

### Interface utilisateur {#user-interface-4}

* Lorsque le sélecteur de date est un champ dans un groupe de plusieurs champs, les valeurs enregistrées dans les champs de date ne sont pas conservées lors de la modification du composant. NPR-20077 : correctif pour GRANITE-19147
...
* La propriété ImageMap existe toujours même après la suppression des images du composant de l’éditeur statique Plusieurs images. NPR-20142 : correctif pour CQ-4222982
...
* Le formulaire prérempli ne fonctionne pas lorsque le formulaire mobile est configuré pour s’afficher en tant que PDF. NPR-20566

**Gestion des droits**

* La boîte de dialogue Sélection de certificat d’authentification mutuelle doit afficher les certificats avec l’utilisation de clé améliorée (EKU) comme authentification du client ou connexion à la carte intelligente. NPR-20708
...
* Correctifs dans la valeur `Resourcetype` dans le nœud de zone de travail.
...
Cette expérience fournit un nouveau Dynamic Media qui se connecte à la nouvelle infrastructure de publication Dynamic Media pour une réplication plus rapide et plus évolutive.
...
* Un message d’erreur apparaît lors de la publication de plusieurs ressources, même si toutes les ressources sont publiées correctement. NPR-19629 : correctif pour CQ-4219611
...
* La page du site n’est pas affichée lorsqu’elle est affichée à l’aide d’un alias dans SiteAdmin. NPR-20053 : correctif pour CQ-4221478
...
* Activation de la publication des balises d’AEM vers Brand Portal à partir de la console tagadmin/tagging. NPR-20271
...
* Amélioration des Formulaires adaptatifs pour joindre un PDF comme pièce jointe dans Safari. Pour prendre en charge la même fonctionnalité dans les formulaires existants, modifiez la configuration dans le widget de pièce jointe et dans &quot;Types de fichiers pris en charge&quot; mettez à jour la valeur application/pdf au lieu de .pdf. NPR-19623

#### Forms Manager {#forms-manager-1}

* Si validationState n’est pas défini sur un champ de formulaire adaptatif et que l’événement elementFocusChanged se produit, un événement d’erreur (errorState) est renvoyé au serveur Adobe Analytics. NPR-19513

### Programme d’installation de Forms JEE {#forms-jee-installer-11}

#### Base {#core-3}

* Le gestionnaire de connexions n’est pas disponible pendant la fermeture. JBoss® supprime la dépendance JDBC avant l’annulation du déploiement de l’EAR de l’auteur ou de l’autrice, ce qui entraîne des problèmes de corruption. NPR-19703
...
* Désactivation du bouton d’envoi en cas de clic pour empêcher l’envoi de plusieurs formulaires sur une page de traduction.
...
* Le service de notification de workflow ne code pas les liens dans les emails, ce qui les empêche de se charger une fois que les utilisateurs les ont cliqués. NPR-19490 : correctif pour CQ-4218055
* Impossible de charger une liste complète de pages/ressources en mode Colonne à l’aide de Chrome. NPR-19458 : correctif pour CQ-4214248
* L’icône Heure de désactivation incorrecte s’affiche dans la boîte de réception AEM lors de l’activation du workflow &quot;Demande d’activation&quot;. NPR-19365 : CQ-4216174
* Problèmes de tri dans la vue Liste. NPR-19217 : CQ-95602
* Lors de la modification du titre ou de la miniature dans les paramètres du dossier de ressources, le groupe d’origine et les autorisations du dossier sont remplacés. NPR-19283 : correctif pour CQ-4216080
...
* Problèmes liés à la présence de plusieurs composants de l’éditeur de texte enrichi dans une boîte de dialogue. NPR-19311 : NPR-19587
* La purge automatique des versions dans la version Vanilla AEM 6.2 ne fonctionne qu’une fois que VersionManagerImpl a été initialisé. NPR-19315 : correctif pour CQ-4217175
* L’instance de workflow est bloquée sur l’étape de workflow &quot;Salesforce.com Export&quot;. NPR-19222 : correctif pour CQ-4212976
* Les pages de copies de langue créées à partir de Live Copies ne sont pas modifiables. NPR-18967
...
* Le service Adobe Email Template ajoute des balises aux modèles d’utilisateurs personnalisés. NPR-19190 : correctif pour CQ-4217113
...
* Un rendu rapide des miniatures en raison d’articles en taille réelle dans AEM console d’articles. NPR-19086 : CQ-4217148
...
* Les utilisateurs ne peuvent pas filtrer les notifications dans la boîte de réception par utilisateur/groupe dans l’interface utilisateur classique de la boîte de réception. NPR-19122 : correctif pour CQ-4215374
...
* Lorsque le contenu est copié de Microsoft® Word ou d’un navigateur Web vers l’éditeur de texte de Correspondence Manager, le style est perdu. NPR-19530
* Le contenu sans saut de ligne ne peut pas être encapsulé dans l’éditeur de texte. NPR-19481
...
>* Les résultats de recherche ne sont pas mis en surbrillance lorsqu’une partie du mot recherché est dans un style différent ou contient un caractère spécial comme &quot; ou &quot; ou \&quot;.
...
* L&#39;écran administrateur de Microsoft® Windows affiche le numéro de version 6.0 après l&#39;installation de CFP6. Correctif pour CQ-4217573
...
* Changement de comportement lors de l’affichage des titres sur la carte Image pour une image dont la propriété dc: title est définie sur String (multifield).
* Correction d’un problème de performances lié aux Cloud Service Dynamic Media, à l’interface utilisateur tactile et aux interfaces utilisateur de sécurité.
...
* La prise en charge du fichier de bibliothèque Target AT.JS, une bibliothèque d’implémentation pour l’intégration côté client avec Adobe Target, est conçue pour les implémentations web classiques et les applications d’une seule page.
* Amélioration des performances d’AEM grâce à la nouvelle fonctionnalité de délai d’expiration de connexion configurable par l’utilisateur ou l’utilisatrice pour Analytics, DTM et Target.
...
* Les boutons d’action Publish sont visibles même lorsque l’utilisateur ne dispose pas de l’accès à la réplication. NPR-17353 ; correctif pour CQ-4209269
* L’administrateur de site et l’administrateur divers ne fonctionnent pas lorsque la minification est activée à l’aide de min:gcc;obfuscate=true. NPR-18593 ; correctif pour CQ-4209220
...
* L’application d’autorisations pour les macros DM interrompt l’affichage pour l’utilisateur administrateur. NPR-18544 ; correctif pour CQ-4211729
* La fonction Publier ultérieurement pour les ressources génère une exception Illegal ArgumentException. CQ-4214532

### Sites {#sites-12}

* Sur un cluster d’auteur actif-actif avec MongoDB, les deux auteurs tentent de déclencher la réplication pour le même contenu, lorsque le temps atteint l’ensemble à l’heure pour le contenu. NPR-18708 ; correctif pour CQ-4210982
...
* Le programme d’installation JCR ne met pas à jour la version du lot après l’installation du package. NPR-18728 ; correctif pour NPR-15135
...
* Demande de résolution Apache Felix Http Bridge dans un environnement AEM. NPR-18297
...
* Activation de la publication des balises d’AEM vers Brand Portal à partir de la console tagadmin/tagging. CQ-4212165
...
* Si une déclaration XML existe dans un fichier XML de données et que le rendu de lettre est initialisé par le biais d’une requête de POST, la lettre correspondante ne parvient pas à afficher les données. NPR-18870
...
>
>* Copier le collage de Word/Web dans l’éditeur de texte de CM affiche le contenu du saut de ligne. NPR-19530
>* Le contenu sans saut de ligne dans l’éditeur de texte de CM ne peut pas être encapsulé. NPR-19449
>

#### Formulaires adaptatifs {#adaptive-forms-9}

* Lors de l’ajout d’un panneau pour les panneaux répétables, la valeur du champ déroulant du panneau précédent est supprimée. NPR-18772
...
* Un script permettant de modifier le titre du bouton au niveau de l’événement initializer du panneau racine du guide ne fonctionne pas. NPR-18476
* La barre de défilement ne s’affiche pas dans le panneau de droite pour les règles créées sous l’éditeur de règles. NPR-18716
...
* Gestion efficace des composants masqués en mode de mise en page dans la tablette.
* Présentation des actions rapides sur les appareils hybrides.
* Résolution des problèmes de synchronisation au niveau des composants avec les Live Copies.

### Ressources {#assets-13}

* Un client est bloqué lorsqu’un utilisateur qui ne dispose pas de l’autorisation requise tente de déplacer l’opération sur une ressource. NPR-18330 ; correctif pour CQ-4212560
...
* Les actions/workflows de passage en caisse ne sont pas disponibles à partir de la console Frise chronologique une fois approx. 80 fragments sont ajoutés au dossier Assets. NPR-18257 ; correctif pour CQ-4211214 et NPR-18251 ; correctif pour CQ-4211216.
...
* Une miniature du PDF n’est pas générée. NPR-17831, NPR-17750 ; correctif pour CQ-4210547
* La tâche de notification d’expiration de la gestion des actifs numériques Adobe CQ ne désactive pas les ressources expirées. NPR-17666 ; correctif pour CQ-107766
...
* Personalization ne fonctionne que pour la première adresse IP lorsque la règle de segmentation est définie pour une plage d’adresses IP. NPR-18121 ; correctif pour CQ-83767
* La connexion échoue en raison de NumberFormatException lorsque la propriété historyShow est activée. NPR-18073 ; correctif pour CQ-101965
...
* Vous ne pouvez pas sélectionner une image après la suppression d’une autre image sur la page. NPR-17711 ; correctif pour CQ-4201323

### Plateforme {#platform-11}

* Les commandes de l’interface utilisateur tactile sont masquées pour les utilisateurs disposant des autorisations requises. NPR-17945 ; correctif pour CQ-4211231
* Balises japonaises manquantes dans le champ de sélection de balise. NPR-17768 ; correctif pour CQ-4210456
* La requête getsize() renvoie des résultats incorrects lorsque FastQuerySize est activé. NPR-18018
* La console web sur l’instance de secours n’est pas accessible. NPR-17861 ; correctif pour Granite-14582

### Commerce {#commerce-2}

* La traversée des requêtes se produit lorsqu’aucune condition n’est définie pour une section pour un plan directeur de catalogue. NPR-18229 ; correctif pour CQ-4211924
...
* Case à cocher manquante afin que vous puissiez rétablir l’héritage. NPR-17753 ; requête de correctif pour CQ-4210139
...
* L’envoi de la page Ressource ne doit pas être nécessaire pour un projet de traduction tant que le projet n’est pas terminé. NPR-18249 ; correctif pour CQ-4209908

### Composants WCM-Foundation {#wcm-foundation-components-8}

* Impossible d’utiliser le composant Iparsys de la fondation WCM dans les modèles modifiables. NPR-18223 ; correctif pour CQ-4210384
...
* Lorsqu’un composant d’image HTL est rendu, le nom de fichier dans l’URL est renommé, ce qui entraîne une URL rompue. NPR-17908 ; correctif pour CQ-4211587
...
* Le dictionnaire de données est lu à plusieurs reprises pendant le rendu de lettre. NPR-18482, correctif pour CQ-4210805
...
* Des erreurs inattendues s’affichent dans les journaux lors de l’installation d’un module complémentaire. NPR-18295
...
* Impossible de signer un document de PDF avec un module de sécurité matérielle distant. NPR-18017
...
* Correction de plusieurs problèmes liés aux composants et workflows de gestion de contenu web, tels que Dépublier et Purge de version.
* Amélioration de la réactivité de la barre d’actions lors de l’affichage des actions de la barre d’outils et des composants Coral.
...
* Dans les étapes de workflow de gestion des ressources numériques, les flux binaires acquis à partir de Session ou ResourceResolver doivent être fermés dans un bloc final. Cela permet de s’assurer que les fuites de ressources ne se produisent pas. NPR-17385 ; correctif pour CQ-4209452
...
* Les utilisateurs disposant d’autorisations &quot;Lecture seule&quot; sur l’application AEM Mobile ne peuvent pas prévisualiser le contenu à partir de la page de gestion de contenu AEM Mobile. NPR-17390 ; correctif pour CQ-4209690
...
* Pour un formulaire adaptatif avec pièce jointe, les entrées en double pour les balises afSubmissionInfo sont créées dans le fichier XML envoyé lorsque le formulaire est envoyé pour la deuxième fois. NPR-17364
...
* La méthode d’API `com.adobe.icc.services.api.LetterInstanceService` ne permet pas l’import. NPR-17922, NPR-16008
...
* L’interface utilisateur de Correspondence Management ne se lance pas lorsqu’une action d’envoi d’HTML utilise la méthode du POST. NPR-17595

**Gestionnaire de formulaires**

* Lorsqu’un formulaire adaptatif est configuré pour les tests AB, cliquer sur Démarrer AB Testing ne lance pas le test et renvoie une erreur de console du navigateur. NPR-17838
...
* L’utilisation d’AEM Forms 6.2 Output Service pour fusionner un formulaire spécifique avec un XML de données prend 20 fois plus de temps. Il est maintenant corrigé dans les environnements Windows et Linux®. NPR-17501
...
* Les fonctionnalités des utilisateurs de formulaires, que ce soit à l’aide de la console d’administration de JEE ou de la console OSGi, doivent être les mêmes. NPR-17670
...
* Lors de la modification de fragments dans l’éditeur de texte lors de l’aperçu d’une lettre, le texte traité doit s’afficher pour modification au lieu des conditions intégrées utilisées dans les fragments. NPR-15748, NPR-17504
...
* Le nombre de résultats affichés sur l’écran Gestion des utilisateurs est différent en mode Carte et Liste. NPR-16241 ; correctif pour GRANITE-16914
...
* Les fichiers `Camera RAW` (y compris ARW, CR2, NEF, DNG et EPS), lorsqu’ils sont ajoutés comme rendu dans les ressources, ne peuvent pas être sélectionnés ni supprimés. Ces fichiers sont automatiquement téléchargés lorsque l’utilisateur clique dessus. NPR-16949 : correctif pour CQ-4206846
* La création d’un PDF dans un autre PDF de l’interface utilisateur d’Assets n’affiche pas les PDF créés dans l’interface utilisateur de gestion des actifs numériques. Toutefois, les PDF sont visibles dans le référentiel CRX. NPR-16833 : correctif pour CQ-4206501
...
La console de workflow Projets affiche une exception NullPointerException sur la page lorsque les workflows sont purgés. NPR-17017 : correctif pour CQ-4194269

### Sites {#sites-15}

* Les fichiers dans `ContextHub` ne sont pas réduits sur l’instance de création. NPR-17022 : correctif pour CQ-79456
* La promotion de lancement WCM-Lancements prend beaucoup de temps pour promouvoir un lancement qui consiste en une grande arborescence d’une page. NPR-16480 : correctif pour CQ-82731
...
* Lors du test AEM modèles de campagne et de la modification des adresses de contrôle pour inclure des &quot;données additionnelles&quot;, la liste déroulante Adobe Campaign disparaît dans le ContextHub de l’interface utilisateur tactile. NPR-16771 : correctif pour CQ-105748

### Mobile on-demand {#mobile-on-demand-3}

* Lors de l’aperçu d’une publication à partir d’AEM Author, une action de contrôle en amont de plus de cinq secondes provoque un pic inhabituel sur le tableau de bord Splunk d’intégration AEM - AEM PECS avec un grand nombre de demandes d’état. NPR-16908 : correctif pour CQ-4207055
...
* Aucune validation de type MIME n’est réalisée lorsque les binaires des fichiers sont chargés vers AEM. NPR-16617
...
* La liste déroulante Police ne renseigne pas la famille de polices lors de la création d’un module de texte basé sur un dictionnaire de données à l’aide d’Internet Explorer. NPR-16944
...
* Pendant la mise à niveau, Configuration Manager échoue avec le délai d’expiration. NPR-16774 (voir [Configuration du délai d’expiration pour les opérations au niveau du composant](install-cfp-aem-forms-jee.md#configuring-timeout-for-operations-at-component-level-npr)).
...
* Correctifs permettant d’utiliser les audiences de synchronisation de ciblage, de publier des campagnes et de sélectionner Mesure de l’objectif dans l’interface utilisateur tactile.
* Résout des problèmes de synchronisation pour les tâches de traduction
* Renforce la sécurité du service de préremplissage Forms
* Améliorations du composant brouillons et envois du portail Forms et du service Forms Barcoded
...
* Dans l’écran Sites de l’interface utilisateur tactile, la tentative de collage d’une page à l’intérieur d’une page sans sous-pages ne fonctionne pas, car le bouton Coller n’est pas disponible. NPR-15894 : correctif pour CQ-4201696.
* Lors du défilement de l’onglet Pages dans le panneau de l’outil de recherche de contenu, quelques jeux de pages s’affichent indéfiniment dans l’interface utilisateur classique, tandis que l’interface utilisateur tactile affiche un ensemble limité de pages non répétées. NPR-16271 : correctif pour CQ-4202371
...
* Lorsqu’un dossier est créé sous la hiérarchie de contenu avec un titre contenant des caractères spéciaux, la version codée de ces caractères s’affiche. NPR-15935 : correctif pour CQ-4202987
* L’utilisateur ne peut pas charger de ressources ni créer de dossiers lors de la navigation dans les ressources en raison d’un comportement incohérent affiché lors de l’utilisation du bouton &quot;Créer&quot;. NPR-16410 : correctif pour CQ-4204793
...
* Lors de l’activation de l’authentification par proxy avec l’authentification Digest, le composant Recherche AEM renvoie une exception ConcurrentModificationException. NPR-15309 : correctif pour CQ-4199191
* Lors de la création d’une activité de test A/B Target dans AEM, l’audience ne se synchronise pas avec Target et n’affiche &quot;aucune audience&quot;. NPR-16229 : correctif pour CQ-4204210
...
* Lors de l&#39;utilisation du ciblage, la publication de la campagne ne publie pas automatiquement l&#39;intégralité de l&#39;arborescence, y compris la marque et la partie principale. NPR-15855 : correctif pour CQ-94630

### Traduction {#translation-8}

* La tâche de traduction de synchronisation est maintenant déclenchée automatiquement et AEM interrogation se produit maintenant sans interroger les projets de traduction. NPR-15163 : correctif pour CQ-90856
...
* Lors de l’enregistrement d’un fragment à l’intérieur d’un fragment dans l’éditeur de formulaire, la racine du modèle de fragment ne renseigne pas la valeur du fragment enfant. NPR-15943
...
* Pour les widgets de pièce jointe qui autorisent plusieurs pièces jointes, l’envoi d’une nouvelle instance de formulaire avec une pièce jointe sur un widget qui comporte déjà une pièce jointe précédente entraîne l’affichage d’un code d’erreur. Cette erreur se produit lors de l’ouverture de la pièce jointe ajoutée au lieu du contenu réel. NPR-16258
* Protection du service de préremplissage de formulaires contre un accès non autorisé par le biais de protocoles tels que `file://`, `http://` et `ftp://`. Voir [Configuration du service de préremplissage à l’aide de Configuration Manager](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-15414

* Requête pour effectuer le rendu du formulaire adaptatif au format PDF au lieu d’un HTML à l’étape de vérification et ajouter toutes les pièces jointes au PDF, de sorte que l’impression affiche le formulaire complet. NPR-9011

**Correspondence Management**

* Lorsque vous travaillez avec un fragment de texte dans une lettre en mode Aperçu ou Édition , si le texte est converti en liste, la fonctionnalité de lettre entière est interrompue. Une erreur JavaScript est également générée. NPR-16460
* L’importation d’un fichier XSD pour créer un dictionnaire de données dans le noeud Correspondence Management échoue, car le navigateur ne répond plus chaque fois que le fichier XSD est chargé et que le noeud racine est sélectionné. Ce problème est indépendant du navigateur et n’apparaît pas dans les journaux du serveur. NPR-16452
...
* Si un champ flottant apparaît en tant que champ d’entrée à partir d’un fragment XDP, le champ ne s’étend pas dans l’aperçu de lettre lors de l’utilisation du navigateur Internet Explorer. NPR-16367
...
* Lorsque vous travaillez sur un fragment de document texte à l’aide du navigateur Internet Explorer, la tentative de mise en retrait du texte échoue, car le curseur n’autorise pas la mise en retrait du texte. NPR-16128
* L’ajout ou la modification d’un dictionnaire de données à un fragment de document texte existant prend beaucoup de temps et l’utilisateur n’en est pas toujours informé. NPR-16102
* Lors de la prévisualisation d’une lettre comportant du contenu défilant à l’aide du navigateur Internet Explorer, la barre de défilement du navigateur chevauche la barre de défilement de la lettre. Par conséquent, le contenu entier ne peut pas être affiché pour les fragments sur le côté droit. NPR-16068
* Lors de la création ou de la modification de fragments de document texte à l’aide du navigateur Google Chrome, la liste déroulante de sélection de couleurs s’affiche automatiquement et ne peut pas être supprimée. L’utilisateur doit sélectionner la liste comme type de saisie de données pour pouvoir modifier le fragment. NPR-16067
...
* Le type de liaison de données dans les lettres lors de la recréation est affiché comme &quot;Utilisateur&quot; même s’il a été défini différemment auparavant. NPR-16619
...
* La sécurisation des formulaires préremplit le service d’accès non autorisé par le biais de protocoles tels que &quot;file://&quot;, &quot;http://&quot; et ftp://. Pour plus d’informations, voir [Configuration du service de préremplissage à l’aide de Configuration Manager](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-15434
...
* Lors de la suppression d’une ressource dans Correspondence Management, un message d’avertissement doit être consigné dans le fichier error.log. NPR-13225
* Améliorez la fonctionnalité de recherche lors de la prévisualisation d’une lettre dans Correspondence Management, afin d’activer la recherche de texte dans le contenu du fragment de texte, au lieu de rechercher uniquement le titre ou le libellé de la lettre. NPR-16103
...
Les principaux points forts du pack de correctifs cumulatif 2 sont les suivants :

* Correctifs de stabilité et améliorations des performances de la plateforme AEM, y compris la structure et les opérations Sling
...
* La valeur des balises avec la propriété `tagBasePath` n’est pas récupérée dans la boîte de dialogue de l’interface utilisateur classique lors de l’accès à un composant de page. NPR-15543 : correctif pour CQ-4199950
...
* Un utilisateur sélectionne un noeud dans CRXDE (par exemple, `/content/dam/`). Cliquez ensuite sur l’onglet &quot;Contrôle d’accès&quot;, en vous assurant qu’il existe une liste de contrôle d’accès. Désormais, le fait de faire glisser et de déposer certains éléments déplace les éléments, pas ceux sélectionnés. NPR-15696 correctif pour GRANITE-16300
...
Note : ce correctif nécessite Oak CFP 1.4.11 ou version ultérieure.
...
* L’état de modification de la boîte de réception de workflow dans le noyau de la gestion de contenu web ne change pas si un utilisateur modifie une page après l’activation d’un workflow. NPR-15684 : correctif pour CQ-4196974
* Le module Ancre de l’éditeur de texte enrichi pour l’interface utilisateur tactile génère un HTML non conforme5 lorsque l’utilisateur clique sur l’icône d’ancrage et ajoute un nom. Il doit ajouter un attribut « id » au lieu de « name » dans la balise HTML5 pour l’élément d’ancrage. NPR-15650 : correctif pour CQ-89782
* Lorsqu’un schéma de métadonnées comportant de nombreux champs est créé et appliqué aux métadonnées de fragment de contenu, aucune barre de défilement n’est créée dans l’écran de métadonnées de fragment de contenu, ce qui rend les champs non modifiables. NPR-15478 : correctif pour CQ-4202622
...
* Dans l’interface utilisateur de l’éditeur de fragments de contenu, lorsque des variantes d’un fragment de contenu sont créées, le panneau latéral n’affiche pas la barre de défilement pour parcourir toutes les variations. NPR-15445 : correctif pour CQ-4199444
...
* Le composant Diaporama de l’interface utilisateur classique ne peut pas créer de diapositives ni faire glisser-déposer des images pour créer des diapositives. NPR-15281 : correctif pour CQ-4194164
* Les utilisateurs et utilisatrices, indépendamment de leur autorisation, peuvent voir les éléments de menu « Créer » telles que Créer une page, Créer un site, Créer une Live Copy, Créer un lancement et Créer un catalogue dans l’Admin Console de site. NPR-15278 : correctif pour CQ-94436
...
* Si une esperluette (&#39;&amp;&#39;) est incluse dans le nom d’un dossier de ressources, le nom du dossier ne s’affiche pas correctement lors de l’accès à la ressource. NPR-15775 : correctif pour CQ-4201735
...
* Lors de la navigation dans Assets et de l’utilisation du mode d’affichage &quot;Colonnes&quot;, si l’utilisateur actualise la page après avoir sélectionné et cliqué sur une ressource, les détails de la ressource s’affichent à la place du contenu actualisé. NPR-15768 : correctif pour CQ-4201727
...
* L’accès à l’interface utilisateur &quot;Mes partages de lien&quot; à l’aide du navigateur Firefox n’affiche pas les éléments ou les utilisateurs partagés et l’écran est inutilisable. NPR-15539 : correctif pour CQ-4200992
* Lors de l’utilisation de Digital Asset Manager, si une page est associée à un ensemble d’images, le déplacement des images vers un nouveau dossier rompt l’association de la page. Par conséquent, certaines des images manquent dans la page associée. NPR-15538 : correctif pour CQ-111479
...
* Malgré l’échec du chargement d’une ressource sur Brand Portal en raison d’une réponse d’erreur du serveur, le statut est mis à jour sur « Publié » sur l’interface utilisateur Brand Portal, ce qui rend difficile d’assurer le suivi du fichier manquant. NPR-15442 : correctif pour CQ-4197968
...
* Lors de l’utilisation de la console Asset Finder en mode Colonnes, la création d’un dossier échoue une fois qu’il a réussi à effectuer une nouvelle tentative. NPR-15370 : correctif pour CQ-4199448
* Si une ressource ou un dossier sélectionné dans l’interface utilisateur de gestion des actifs numériques comporte une virgule dans le nom, l’onglet Références n’est pas disponible et affiche le message &quot;La liste des références n’est pas disponible pour les sélections multiples&quot;. NPR-15362 : correctif pour CQ-4199721
...
* Après la mise à niveau d’AEM 6.2 vers le pack de services 1, la synchronisation des ressources dans Scene7 échoue. La propriété `dam:Scene7FileStatus` affiche le statut « `UploadFailed` » même pour les ressources publiées. NPR-15269 : correctif pour CQ-4197708
...
* L’exécution du workflow Scene7 entraîne l’ouverture de fichiers qui ne se ferment pas. Demande d’amélioration du service AEM-S7 afin qu’il conserve et réutilise une seule instance HttpClient avec une configuration de pool partagée. NPR-15357 : correctif pour CQ-109958

### Traduction {#translation-9}

* Lors de l’utilisation de projets de traduction, la mise à jour de copies de langue à partir de la racine principale anglaise génère 11 lancements distincts portant le même nom et la même racine source. Cependant, ils ont chacun des racines de lancement légèrement différentes, au cas où le nom de page suivrait un modèle défini. NPR-15605 : correctif pour CQ-4200699
...
* Lors du stockage des brouillons et des envois de formulaires adaptatifs dans la base de données, l’état du formulaire adaptatif est corrompu lorsque la connectivité de la base de données échoue pour une raison quelconque (par exemple, après une longue période d’inactivité). NPR-15297

#### Programme d’installation de Forms JEE {#forms-jee-installer-18}

`**Core**`

* Après la mise à niveau vers la dernière version de Java™ 1.8.0_121-b13, l’interface d’utilisation d’administration n’est pas accessible dans AEM Forms. NPR-15330

`**XTG**`

* Lors de l’utilisation du service Output pour fusionner un formulaire spécifique avec un fichier dataxml, le temps de réponse est 20 fois supérieur au temps pris par un serveur ES4 SP1. NPR-15283

#### Application AEM Forms {#aem-forms-app-1}

* Lors de la récupération de tâches non enregistrées, le message affiché doit être plus clair afin de réduire les erreurs pour les utilisateurs et utilisatrices. NPR-15377
...
* Améliore la gestion des balises dans les collections dynamiques.
* Contrôles d’accès plus stricts pendant les opérations CRUD sur les dossiers
...
* Demande de mise à niveau du coffre de fichier Jackrabbit vers la version 3.1.30 pour résoudre plusieurs problèmes. NPR-13454
* Une erreur de cache se produit lorsque la distribution Sling synchronise les modules de distribution de l’auteur à la publication. NPR-13034 : correctif pour GRANITE-13970
...
* Lorsque Sightly Parsys est utilisé pour le rendu des composants insérés via JavaScript dans l’interface d’utilisation tactile, la décoration personnalisée est ignorée une fois la page actualisée. NPR-14122
* Les listes déroulantes de Target ne fonctionnent pas dans la boîte de dialogue de l’interface utilisateur tactile lors de la création de plusieurs champs de texte enrichi, tels que des liens. NPR-13911
* Lors de la modification d’un champ de texte avec plusieurs propriétés RTE (éditeur de texte enrichi) dans une boîte de dialogue (IU tactile), la cible d’action passe aléatoirement sur une propriété RTE spécifique. NPR-13703
* Le composant vidéo prêt à l’emploi par défaut n’affiche pas la miniature vidéo. NPR-14976
* Les informations sont lentement chargées dans l’onglet Utilisations en direct de l’éditeur de modèles. NPR-14880 : correctif pour CQ-83417
...
* Le workflow Décompresser ne peut pas gérer les images dont le nom de fichier image contient des caractères spéciaux. NPR-15227 : correctif pour CQ-103887
* Les ressources dont l’expression de condition s’accompagne de Répéter ne sont pas affichées correctement. Lorsque l’utilisateur ou l’utilisatrice affiche la prévisualisation du modèle de lettre `*CDN3835RLCEN*`, aucune ressource située dans la zone cible du corps n’est affichée. Pour les widgets de pièce jointe qui autorisent plusieurs pièces jointes, l’envoi d’une nouvelle instance de formulaire avec une pièce jointe sur un widget qui comporte déjà une pièce jointe précédente entraîne l’affichage d’un code d’erreur. NPR-14844
...
* Les requêtes de recherche de ressources s’exécutent lentement dans l’interface utilisateur tactile lors de recherches simultanées par plusieurs utilisateurs. NPR-15019 : correctif pour CQ-4195405
...
* Les utilisateurs ne peuvent pas supprimer une recherche enregistrée ou une collection dynamique. NPR-14924 : correctif pour CQ-108494
...
* Les utilisateurs disposant de droits de réplication peuvent désormais supprimer des dossiers de ressources. NPR-14321 : correctif pour CQ-88271
* Lors de la tentative de modification des profils vidéo d’une vidéo dans l’éditeur de canaux, la boîte de dialogue de conception ne s’ouvre pas et génère une exception de pointeur nul dans le journal des erreurs. NPR-14144 : correctif pour CQ-81101
* La propriété d’horodatage &quot;Créé&quot; générée par le système affichée dans la page des propriétés d’une ressource est incorrecte. NPR-13992 : correctif pour CQ-95029
...
* Les utilisateurs ne peuvent pas modifier les métadonnées des ressources en bloc à partir de la page des propriétés. NPR-13721 : correctif pour CQ-100703
...
* AEM Assets ne peut pas trier plus de 50 ressources par taille à la fois dans la vue Liste lorsque le dossier contient de nombreuses ressources. CQ-100588
* La sélection de plusieurs ressources génère une erreur avec le code de réponse 414 (URI de requête trop long) si l’URI de ressource/dossier est trop long. NPR-13516 : correctif pour CQ-76076
* La page Rapport de ressources ne répond plus lorsque l’utilisateur ou l’utilisatrice sélectionne toutes les options dans la boîte de dialogue `Configure Columns`. NPR-13187 : correctif pour CQ-95589
...
* Les ressources dont les noms comportent des caractères spéciaux ne peuvent pas être téléchargées à partir du lien de téléchargement partagé par e-mail. NPR-12872 : correctif pour CQ-95795
* Les rapports de ressources prêts à l’emploi générés lorsqu’un nombre important de ressources est important entraînent de lourds parcours où la recherche n’atteint aucun pic d’index et d’utilisation du processeur. NPR-12811 : correctif pour CQ-84409
* Les utilisateurs disposant d’un accès à l’instance d’auteur AMS AEM Assets à partir de réseaux disparates ne peuvent pas charger de ressources à l’aide du chargement par blocs sans droits de suppression sur les dossiers. NPR-12768 : correctif pour CQ-82715
...
* Lors de l’exportation d’un projet de traduction créé avec la méthode de traduction définie sur &quot;humain&quot; et le fournisseur de traduction sur &quot;aucun&quot;, aucun fichier translation_export_summary.xml n’est généré, car le fichier de mappage du GUID est manquant. NPR-13137 : correctif pour CQ-91976
...
* Le lien de la payload dans l’onglet Archive de la console Processus renvoie une erreur avec le code de réponse &quot;404&quot; en cliquant dessus. NPR-14993 : correctif pour CQ-4194977
...
* De nombreux traitements Sling `AEM-Scene7` qui ont été créés et annulés lors de l’activation d’AEM sont consignés comme traitements d’archivage pendant la réplication. NPR-12835 : correctif pour CQ-86115
...
* Si vous définissez une valeur par défaut pour un bouton radio et sélectionnez une autre valeur lors de l’enregistrement ou de l’envoi, le préremplissage n’affiche pas la valeur sélectionnée. NPR-15304
* Google Chrome affiche un comportement incorrect lors de l’utilisation de l’événement options pour remplir de manière dynamique la liste déroulante et envoyer la valeur de l’élément sélectionné. NPR-15198
...
* Lors de l’utilisation du mode d’édition pour les formulaires adaptatifs, des erreurs JavaScript telles que &quot;Le gestionnaire du composant n’est pas valide&quot; et &quot;guidelib is undefined&quot; sont rencontrées. NPR-15112
...
* Le panneau Éléments de données de l’utilitaire Expression Builder continue de charger au cas où un module de condition serait créé sans sélectionner de dictionnaire de données. NPR-14688
...
* Lors de l’export en bloc des ressources de Correspondence Management depuis l’interface d’utilisation, le serveur AEM Forms génère des journaux inutiles. NPR-15226
* Lorsqu’une lettre est prévisualisée, le texte justifié s’affiche dans une police différente. NPR-15468

`**Forms Portal**`

* Les pièces jointes aux formulaires envoyés sur le portail Formulaires ne sont pas visibles lorsqu’un nouveau brouillon est envoyé à partir de l’envoi depuis le portail. NPR-13515
...
* L’événement d’audit associé à la connexion de l’utilisateur ou de l’utilisatrice est consigné avec une heure non valide. La durée correcte d’un événement d’audit n’est pas traçable. NPR-13107
...
* Lorsqu’une tâche de brouillon transférée dans HTML Workspace est renvoyée à l’utilisateur initial, la tâche n’apparaît pas dans la file d’attente de l’utilisateur initial. NPR-15178
...
* Forms Designer se bloque lorsque l’utilisateur exécute l’utilitaire de vérification orthographique avec le paramètre régional de formulaire sélectionné Français (Canada). NPR-13740
...
* Lorsqu’un formulaire adaptatif est créé à partir d’un fichier XDP, la tabulation pour l’accessibilité ne suit pas de modèle défini. NPR-12562
...
>[!NOTE]
>
>Pour les clients AEM Forms, il est essentiel d’installer le module complémentaire d’AEM forms après l’installation d’AEM Service Pack, de Cumulative Service Pack ou de Feature Pack.

Vous pouvez télécharger le package CFP directement à partir de la distribution logicielle ou effectuer les étapes suivantes :
...
* Le pack de services 1 d’AEM 6.2 est une condition préalable à l’installation du CFP. Pour obtenir des instructions sur l’installation, reportez-vous aux notes de mise à jour de la section [Pack de services 1 pour AEM 6.2](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).

* Le pack de correctifs cumulatif peut être téléchargé sur la [Distribution logicielle](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html), accessible directement depuis l’instance AEM.
* Pour un déploiement en grappe utilisant (RDBMK ou MongoDB), le package CFP peut être installé sur n’importe quelle instance d’auteur qui utilise Package Manager.

* Avant d’installer le Cumulative Fix Pack, veillez à prendre un instantané ou à sauvegarder votre instance AEM.
* La désinstallation du CFP n’est pas prise en charge.

### Installer le package CFP à l’aide de de la distribution logicielle {#install-the-cfp-via-package-share}

Pour installer le pack de correctifs cumulatif sur une instance existante d’AEM 6.2 SP1, procédez comme suit :

1. Pour télécharger le package, cliquez sur [Distribution logicielle](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/cumulativefixpack/aem-6.4.8-cfp-2.0.zip).
...
## Paramètres de délai d’expiration configurables par l’utilisateur ou l’utilisatrice pour les connexions DTM, Analytics et Target {#user-configurable-timeout-parameters-for-dtm-analytics-target-search-promote-connections}
...
| **Connexions** | **Délai d’expiration de connexion&#42;** | **Délai d’expiration du socket&#42;&#42;** |
|---|---|---|
| DTM | 30 000 millisecondes | 30 000 millisecondes |
| Analytics | 30 000 millisecondes | 30 000 millisecondes |
| Target | 60 000 millisecondes | 30 000 millisecondes |

* **Délai d’expiration de connexion&#42;** : délai d’expiration en millisecondes jusqu’à ce qu’une connexion soit établie. Une valeur de délai nulle est interprétée comme un délai d’expiration infini.
...
NPR-21355 est inclus dans CFP12.1. Si vous installez CFP12.1 ou une version ultérieure, procédez comme suit pour configurer NPR-21355 sur le serveur d’applications JBoss®. Si vous installez le CFP 12.1 sur un serveur AEM Forms qui s’exécute sur des serveurs d’applications Oracle WebLogic ou IBM® WebSphere, aucune configuration supplémentaire n’est nécessaire :

1. Sauvegardez, supprimez et créez un fichier module.xml. L’emplacement par défaut du fichier est [Répertoire_d_installation_d_AEM_Forms]/jboss/modules/system/couches/base/com/adobe/livecycle/main/
...
* Pour la configuration de grappe du serveur JBoss®, ouvrez le fichier domaine.conf.bat pour le modifier.
...
1. (Système d’exploitation basé sur Linux uniquement) Modifiez les fichiers de configuration [AEM_Forms_Installation_directory]/jboss/standalone.conf ou domain.conf :

* Pour la configuration autonome du serveur JBoss®, ouvrez le fichier standalone.conf pour le modifier.
* Pour la configuration de grappe du serveur JBoss®, ouvrez le fichier domaine.conf pour le modifier.
...
>[!NOTE]
>
>NPR-19778 fait partie du CFP 14.

Le nombre de files d’attente partagées n’est pas actualisé pour les autres utilisateurs lorsqu’un utilisateur revendique une tâche. Par conséquent, Adobe a introduit une nouvelle propriété. Suivez les étapes ci-dessous pour configurer cette propriété sur votre instance AEM :
...
* Changement de comportement lors de l’affichage des titres sur la carte d’image pour les images ayant une propriété `dc:title` définie sur String (multichamp) [] : seul le dernier titre modifié est affiché sur la carte Image dans l’interface d’utilisation, bien que tous les titres soient enregistrés dans CRX. Correctif pour CQ-4217165
...
* Erreur observée lors de l’installation du CFP dans Mongo et DB2® : **org.apache.sling.discovery.oak.TopologyWebConsolePlugin addDiscoveryLiteHistoryEntry : Exception : java.lang.NullPointerException**. Cette erreur ne se produira pas après l’installation d’un CFP au-delà du CFP 8.
* (Tâche de mise à jour du planificateur de maintenance Adobe Granite) com.adobe.granite.maintenance.impl.TaskScheduler : aucune tâche de maintenance trouvée avec le nom WorkflowPurgeTask pour la fenêtre `granite:weekly`.
...
* Lorsque vous installez CFPx sur AEM 6.2 SP1 qui comprend le pack de fonctionnalités Balises intelligentes, l’étape de workflow précédemment ajoutée pour la balise intelligente Assets est supprimée du workflow Ressources de mise à jour de gestion des actifs numériques.

## Uber Jar {#uber-jar}

Uber Jar pour 6.2 SP1-CFP20 est disponible dans le référentiel Maven public Adobe.
...
>[!MORELIKETHIS]
>
>* [Page des correctifs AEM 6.2](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [Notes de mise à jour d’AEM 6.2 SP1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Notes de mise à jour d’AEM 6.2](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Page de produits AEM ](https://business.adobe.com/fr/products/experience-manager/adobe-experience-manager.html)
>* [Documentation d’AEM 6.2](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Mises à jour de produit prioritaires par Adobe](https://experienceleague.adobe.com/fr/docs/release-notes/experience-cloud/current)
