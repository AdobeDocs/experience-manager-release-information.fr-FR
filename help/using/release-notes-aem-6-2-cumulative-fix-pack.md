---
title: Pack de correctifs cumulatif AEM 6.2
...
# Notes de mise à jour du pack de correctifs cumulatif AEM 6.2.{#release-notes-aem-cumulative-fix-pack}
...
Le pack de correctifs cumulatif d’AEM 6.2 SP1-CFP20, dernier pack de correctifs cumulatif pour AEM 6.2, est une mise à jour importante comportant des correctifs client importants publiés depuis la disponibilité générale d’[AEM 6.2 SP1](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
>[!NOTE]
>
>* Un nouveau lot Sling `discovery- api` Johnzon 1.0.0 est inclus avec le pack de correctifs cumulatif d’AEM 6.2 SP1-CFP10. En outre, un utilisateur ou une utilisatrice de service sling-discovery est ajouté avec les autorisations de lecture et d’écriture pour le nœud */var/discovery* dans le référentiel CRX.
>
...
>[!NOTE]
>
>Pour la clientèle AEM Forms, il est impératif d’installer le package complémentaire AEM Forms après l’installation de tout pack de services, pack de correctifs cumulatif ou pack de fonctionnalités AEM.
...
* Lors de la prévisualisation de la lettre au format PDF, un champ de texte placé sur le gabarit de page ne se conforme pas à la valeur saisie dans l’onglet des données ou à la liaison de données spécifiée. NPR-29239 : correctif pour CQ-4266856.
...
* Il est impossible de renommer le dossier DAM en l’absence de l’autorisation de modification d’ACL. NPR-27555 : correctif pour CQ-104652
...
* L’affichage d’une icône permettant de rompre l’héritage n’est pas cohérent. NPR-27671 : correctif pour CQ-4257779
...
* La fonctionnalité de téléchargement exploite AEM Assets par le biais du servlet de téléchargement de ressources, ce qui permet aux utilisateurs et utilisatrices anonymes de télécharger toutes les ressources. NPR-27054, correctif pour CQ-4254732
...
* L’administration de Site classique ne génère pas la liste dans le volet de droite pour certaines pages. NPR-27247 : correctif pour CQ-4251621
...
* En raison du caractère spécial de tiret cadratin, l’éditeur ou l’éditrice ne parvient pas à vider le cache. NPR-27199 : correctif pour CQ-4242790
...
* La barre d’outils de l’éditeur de texte enrichi RTE couvre la liste sélectionnée. NPR-25165 : correctif pour CQ-4248948
...
* Le lot cq-social-as-provider ne démarre pas avec les versions 3.x du pilote Mongo. NPR-26271 : correctif pour CQ-4252710
...
* La variable est renseignée en tant que type de données Liste, le sous-type est Chaîne, mais une erreur « impossible de forcer l’objet » se produit. NPR-26194 : correctif pour CQ-4252287
...
* La création du lancement échoue lors de l’initialisation du workflow « Demander le lancement » sur un projet. NPR-24255 : correctif pour CQ-4245030
...
* Une utilisation intensive du processeur est observée en raison du nombre élevé de requêtes dans la fonctionnalité de recherche d’administrateurs et administratrices. NPR-23588 : correctif pour Granite-21286
...
* Multifield lance NPE lorsque FormData n’est pas présent pour la requête. NPR-24513 : correctif pour Granite-21055
...
* Lorsqu’une lettre est prévisualisée via la prévisualisation par défaut, le mappage des fragments de disposition ne s’affiche pas alors que la même lettre apparaît correctement lorsque vous cliquez sur le bouton de prévisualisation. NPR-22993 : correctif pour CQ-4237745
...
* Mettez à jour le dernier fichier adobe-reader-extensions-dsc.jar pour obtenir des correctifs dans Reader Extension.
* Correction du hook de validation afin de tenir compte des éléments `:invalid` pour l’entrée de numéro de rappel.

### Ressources {#assets-4}

* Les données EmbedXMP sont toujours définies comme « actives » pour le processus de génération Pyramid TIFF. NPR-22776 : correctif pour CQ-4234498
...
* (Firefox) La case à cocher pour la persona sélectionnée n’est pas la bonne. NPR-23345
...
* Lorsqu’une lettre est prévisualisée via la prévisualisation par défaut, le mappage des fragments de disposition ne s’affiche pas alors que la même lettre apparaît correctement quand vous cliquez sur le bouton de prévisualisation. NPR-23335 : correctif pour CQ-4237745
...
* (OSGI) [JEE] Statut de signature incorrect pour le PDF avec pièce jointe lors de la vérification avec « Valider le processus PDF ». NPR-23269, NPR-23737
...
* Les personnes ne peuvent pas télécharger de ressources comportant une clause de non-responsabilité et des noms de fichier longs. NPR-22163 : correctif pour CQ-4235274
* Un guillemet simple empêche la mise à jour des métadonnées dans l’affichage par lots et l’interface d’utilisation est rompue lorsque vous ouvrez les propriétés d’une ressource à l’aide des actions rapides de la barre d’outils. NPR-22317, NPR-22353 : correctif pour CQ-4236990, CQ-4236469
...
* (Digital Publishing Suite) Les champs Date de publication et Date de couverture doivent être définis pour les folios avant leur chargement dans DPS. NPR-22484
...
* Le déploiement d’une structure qui a été déplacée dans une racine principale entraîne un `cq:moveTarget` erroné. NPR-22373 : correctif pour CQ-4232536
...
* (HTML Workspace) Lorsqu’une personne demande une tâche, le nombre de files d’attente est actualisé pour cette personne spécifique uniquement. En d’autres termes, à condition que la page soit actualisée. NPR-19778 : correctif pour CQ-4233665
...
* (Safari) Les personnes ne peuvent pas afficher toutes les ressources dans le volet de recherche de ressources. NPR-21981 : correctif pour CQ-4213720
* La distorsion du temps renvoie l’erreur « RecursionTooDeepException » avec une page altérée et aucune nouvelle version n’est créée, même si la date est modifiée. NPR-21707 : correctif pour CQ-4199536
...
* Les personnes ne peuvent pas voir les valeurs de paramètre statique de test dans l’appel mbox qui s’affiche lors du test avec AT.js comme bibliothèque cliente dans la configuration cloud. NPR-21930 : correctif pour CQ-4234520

### Plateforme {#platform-4}

* Problèmes de performances liés à la synchronisation des personnes lorsque le nombre d’utilisateurs, d’utilisatrices ou de groupes est important. NPR-20431 : correctif pour CQ-4223282
* Personnes non synchronisées à l’aide de la distribution Sling. NPR-21911 : correctif pour Granite-20404
...
* La propriété jcr est manquante pour l’ID d’utilisation d’initiation lors de la création du projet de traduction. NPR-21715 : correctif pour CQ-4230713
...
* Un nombre trop élevé de caractères dans Omnisearch peut provoquer le crash du serveur AEM. NPR-21083 : correctif pour CQ-4223602
...
* Lors de l’ajout d’une structure Cible, le mode Ciblage n’est pas disponible dans la liste du mode de sélection de l’éditeur. NPR-21047
...
* Les bibliothèques de balises personnalisées qui font référence aux balises implémentées en tant que script sont introuvables après la mise à niveau vers AEM 6.3. NPR-20149 : correctif pour Granite-18433
...
* Plusieurs fichiers SWF sont vulnérables au cross-site scripting (XSS). NPR-20612 : correctif pour CQ-4213297
...
* Mise à jour des fichiers Jsafe Jars vers CryptoJ 6.1.3.1 dans Core, Encryption, Signature et Document Security. NPR-21360, NPR-21361, NPR-21356, NPR-21358
...
* Mise à jour des fichiers Jsafe Jars vers CryptoJ 6.1.3.1 dans RE. NPR-21357
...
* Correctifs du composant Sightly et LiveCopies.
...
* Les personnes ne peuvent pas télécharger de ressources comportant une clause de non-responsabilité et des noms de fichier longs. NPR-20255 : correctif pour CQ-4222139
...
* (Interface d’utilisation Correspondance) Désactivation du bouton d’envoi après un clic. NPR-21078
...
* Si le filtre de date modifiée est modifié après l’ouverture de la recherche enregistrée, il n’y a aucun effet sur les résultats. De plus, les résultats affichés sont identiques à la valeur enregistrée précédente du filtre de date modifiée. NPR-19739 : correctif pour CQ-4219425
...
* (IU tactile) La suppression des lancements via la console d’interface d’utilisation classique rend toutes les pages non modifiables. NPR-20520 : correctif pour CQ-4225074
...
* Le script valueCommit est exécuté deux fois pour DropDownList lorsqu’il est modifié via l’interface d’utilisation. NPR-19989 : correctif pour CQ-110212
...
* La boîte de dialogue de sélection des certificats d’authentification mutuelle/CAC devrait afficher les certificats avec une utilisation améliorée de la clé (EKU) comme authentification cliente ou connexion à la carte intelligente. NPR-20708
...
* AEM présente un comportement inattendu lorsqu’une personne tente de publier/`/unpublish/create` une version d’une page verrouillée par une autre personne. NPR-19249 : correctif pour CQ-4215298 et CQ-4203856
...
* Amélioration des Formulaires adaptatifs pour joindre un PDF comme pièce jointe dans Safari. Pour prendre en charge la même fonctionnalité dans des formulaires existants, modifiez la configuration dans le widget de pièce jointe et mettez à jour la valeur application/pdf à la place de .pdf dans « Types de fichiers pris en charge ». NPR-19623
...
* Le gestionnaire de connexions n’est pas disponible pendant l’arrêt. JBoss® supprime la dépendance JDBC avant l’annulation du déploiement de l’EAR de l’auteur ou de l’autrice, ce qui entraîne des problèmes de corruption. NPR-19703
...
* Le service de notification de workflow n’encode pas les liens dans l’e-mail, ce qui empêche leur chargement une fois que les personnes ont cliqué dessus. NPR-19490 : correctif pour CQ-4218055
...
* Le service Modèle d’e-mail Adobe ajoute des balises aux modèles d’utilisation personnalisés. NPR-19190 : correctif pour CQ-4217113

### Projets {#projects-2}

* Les éditeurs et éditrices de projet ne peuvent pas copier/coller de ressources dans le dossier de ressources du projet. NPR-19619
...
* Les personnes ne peuvent pas filtrer les notifications dans la boîte de réception par utilisateur/utilisatrice/groupe dans l’interface d’utilisation classique de la boîte de réception. NPR-19122 : correctif pour CQ-4215374
* La zone cliquable ne conserve pas les coordonnées sélectionnées dans le composant image HTL. NPR-18911 : CQ-4211584
...
* La copie du contenu de Microsoft® Word ou d’un navigateur web vers l’éditeur de texte Correspondance manager ne conserve pas le style. NPR-19530
* Le contenu sans saut de ligne dans l’éditeur de texte ne peut pas encapsuler le contenu. NPR-19481
...
* Les propriétés de schéma XML de formulaires adaptatifs ne peuvent pas être modifiées après l’application de CFP6 à AEM 6.2. Correctif pour CQ-4219684
...
* L’écran d’administration sous Microsoft® Windows affiche la version 6.0 après l’installation de CFP6. Correctif pour CQ-4217573
...
* Changement du comportement lors de l’affichage des titres sur une carte d’image pour les images ayant une propriété dc:title définie sur String (multichamps).
* Correctifs de problèmes de performances avec Dynamic Media Cloud Services, l’interface d’utilisation tactile et les interfaces d’utilisation de sécurité.
...
* Prise en charge du fichier de bibliothèque cible AT.JS, une bibliothèque d’implémentation pour une intégration côté client avec Adobe Target conçue pour les implémentations web classiques et les applications à page unique.
...
* Les boutons d’action de publication sont visibles même si l’utilisateur ou l’utilisatrice ne dispose pas de l’accès de réplication. NPR-17353 ; correctif pour CQ-4209269
* Site admin et Misc admin ne fonctionnent pas lorsque la minimisation est activée à l’aide de min:gcc;obfuscate=true. NPR-18593 ; correctif pour CQ-4209220
...
* Appliquer des autorisations pour les macros DM provoque une altération de l’affichage pour l’utilisateur ou l’utilisatrice disposant des droits d’administration. NPR-18544 ; correctif pour CQ-4211729
* La fonction Publier ultérieurement pour les ressources génère une exception Illegal ArgumentException. CQ-4214532

### Sites {#sites-12}

* Dans un cluster d’instances de création actif-actif avec MongoDB, deux instances de création tentent de déclencher une réplication pour le même contenu lorsque l’heure d’activation du contenu est atteinte. NPR-18708 ; correctif pour CQ-4210982
...
* Requête de résolution d’Apache Felix Http Bridge dans un environnement AEM. NPR-18297
...
* Les zones cliquables ne conservent pas les coordonnées sélectionnées dans le composant image HTL. NPR-18530 ; correctif pour CQ-4211584
...
* Si une déclaration XML existe dans un fichier de données XML et qu’un rendu d’une lettre est démarré via une requête POST, la lettre correspondante n’affiche pas les données. NPR-18870
...
>
>* Un copier-coller à partir de Word/du web vers l’éditeur de texte CM affiche un saut de ligne dans le contenu. NPR-19530
>* Un contenu sans saut de ligne dans l’éditeur de texte CM ne peut pas appliquer un retour à la ligne automatique. NPR-19449
>

#### Formulaires adaptatifs {#adaptive-forms-9}

* Lors de l’ajout d’un panneau à des panneaux répétables, la valeur du champ déroulant du panneau précédent est supprimée. NPR-18772
...
* Un script destiné à modifier le titre du bouton lors de l’événement d’initialisation du panneau racine du guide ne fonctionne pas. NPR-18476
...
* Un client ou une cliente fait l’objet d’un blocage lorsqu’un utilisateur ou une utilisatrice qui ne dispose pas de l’autorisation requise tente une opération de déplacement sur une ressource. NPR-18330 ; correctif pour CQ-4212560
...
* L’action/les workflows de paiement ne sont pas disponibles dans la console Chronologie quand environ 80 fragments sont ajoutés dans le dossier Ressources. NPR-18257 ; correctif pour CQ-4211214 et NPR-18251 ; correctif pour CQ-4211216.
...
* La miniature du PDF n’est pas générée. NPR-17831, NPR-17750 ; correctif pour CQ-4210547
* La tâche de notification d’expiration Adobe CQ DAM ne désactive pas les ressources expirées. NPR-17666 ; correctif pour CQ-107766
...
* Échec de la connexion en raison d’une exception NumberFormatException lorsque la propriété historyShow est activée. NPR-18073 ; correctif pour CQ-101965
* Les pages supprimées marquées sont visibles dans l’interface d’utilisation tactile. NPR-18025 ; correctif pour CQ-86694
...
* Les commandes de l’interface d’utilisation tactile sont masquées pour les utilisateurs et les utilisatrices qui disposent des autorisations requises. NPR-17945 ; correctif pour CQ-4211231
...
* La console web n’est pas accessible dans l’instance secondaire. NPR-17861 ; correctif pour Granite-14582

### Commerce {#commerce-2}

* Une traversée de requête survient lorsqu’un plan directeur de catalogue n’a aucune condition définie pour une section. NPR-18229 ; correctif pour CQ-4211924
...
* Case à cocher manquante pour vous permettre de rétablir l’héritage. NPR-17753 ; requête de correctif pour CQ-4210139
...
* La soumission de la page Ressource ne devrait pas être nécessaire à nouveau pour un projet de traduction tant que le projet n’est pas terminé. NPR-18249 ; correctif pour CQ-4209908

### Composants WCM-Foundation {#wcm-foundation-components-8}

* Impossible d’utiliser le composant de base Iparsys WCM dans les modèles modifiables. NPR-18223 ; correctif pour CQ-4210384
* La zone cliquable ne conserve pas les coordonnées sélectionnées dans le composant image HTL. NPR-18032 ; correctif pour CQ-4211584
* Lors du rendu d’un composant image HTL, le nom de fichier dans l’URL est renommé, ce qui produit une URL incorrecte. NPR-17908 ; correctif pour CQ-4211587
...
* Le dictionnaire de données est lu de manière répétée pendant le rendu d’une lettre. NPR-18482, correctif pour CQ-4210805
...
* Des erreurs inattendues apparaissent dans les journaux lors de l’installation d’un package de module complémentaire. NPR-18295
...
* Impossible de signer un document PDF avec un module de sécurité matérielle à distance. NPR-18017
...
* Les utilisateurs et les utilisatrices disposant d’autorisations de lecture seule sur l’application AEM Mobile ne peuvent pas prévisualiser le contenu de la page de gestion de contenu d’AEM Mobile. NPR-17390 ; correctif pour CQ-4209690
...
* Pour un formulaire adaptatif avec pièce jointe, des doublons d’entrées des balises afSubmissionInfo sont créés dans le XML envoyé lorsque le formulaire est envoyé pour la deuxième fois. NPR-17364
...
* L’interface d’utilisation de Correspondence Management ne se lance pas lorsqu’une action d’envoi HTML utilise la méthode POST. NPR-17595

**Gestionnaire de formulaires**

* Avec un formulaire adaptatif configuré pour AB Testing, cliquer sur Démarrer AB Testing ne démarre pas le test et renvoie une erreur dans la console du navigateur. NPR-17838
...
* L’utilisation d’AEM Forms 6.2 Output Service pour fusionner un formulaire spécifique avec un XML de données prend 20 fois plus de temps. Cela a été corrigé dans les environnements Windows et Linux®. NPR-17501
...
* Les fonctionnalités pour les utilisateurs et les utilisatrices de formulaires doivent être les mêmes en utilisant la console d’interface d’administration JEE ou la console OSGi. NPR-17670
...
* Lors de la modification de fragments au cours d’une prévisualisation de lettre dans l’éditeur de texte, le texte traité doit être affiché pour être modifié au lieu des conditions intégrées utilisées dans les fragments. NPR-15748, NPR-17504
...
* L’ajout ou la personnalisation de colonnes dans les résultats d’**Assets OmniSearch** en superposition dans `/apps` ne fonctionne pas. NPR-16737 : correctif pour CQ-4206785
...
* Le nombre de résultats affichés sur l’écran Gestion des utilisateurs et utilisatrices est différent dans les vues Carte et Liste. NPR-16241 ; correctif pour GRANITE-16914
...
* Les fichiers `Camera RAW` (y compris ARW, CR2, NEF, DNG et EPS), lorsqu’ils sont ajoutés comme rendu dans les ressources, ne peuvent pas être sélectionnés ni supprimés. Ces fichiers sont téléchargés automatiquement lorsque l’utilisateur ou l’utilisatrice clique dessus. NPR-16949 : correctif pour CQ-4206846
* La création d’un PDF dans un autre PDF dans l’interface d’utilisation Assets n’affiche pas les PDF créés dans l’interface d’utilisation DAM. Toutefois, les PDF sont visibles dans le référentiel CRX. NPR-16833 : correctif pour CQ-4206501
...
* La promotion du lancement de WCM-Launches prend beaucoup de temps pour les lancements constitués d’une grande arborescence de page. NPR-16480 : correctif pour CQ-82731
...
* Dans WCM-Launches, les pages associées à un lancement ne sont pas dépubliées lorsque l’action est lancée à partir des propriétés de la page dans l’interface d’utilisation tactile. NPR-16560 : correctif pour CQ-4204681
...
* L’accès au volet **[!UICONTROL Navigation globale]** avec les navigateurs Firefox ou Google Chrome offre une expérience d’utilisation différente. Le navigateur Firefox affiche le menu **[!UICONTROL Outils]**. Le navigateur Google Chrome affiche le menu **[!UICONTROL Navigation]**. NPR-16770 ; correctif pour CQ-4200456

### Campagne {#campaign}

* Lors du test des modèles de campagne AEM et de la modification des adresses de contrôle pour inclure des « données supplémentaires », la liste déroulante Adobe Campaign disparaît de l’interface d’utilisation tactile de Context Hub. NPR-16771 : correctif pour CQ-105748

### Mobile on-demand {#mobile-on-demand-3}

* Lors du contrôle en amont d’une publication depuis l’environnement de création d’AEM, une action de contrôle en amont qui dure plus de 5 secondes provoque un pic inhabituel sur le tableau de bord Splunk AEMM – AEM PECS avec un grand nombre de demandes de statut par seconde. NPR-16908 : correctif pour CQ-4207055
...
* Prise en charge de l’expression régulière de type `dataRef=C:/Users/` dans la **[!UICONTROL Configuration du service de préremplissage par défaut]**. Ce type est destiné au préremplissage des données pour les formulaires adaptatifs. NPR-16425
...
* Les `Upload Comments` pour les pièces jointes ne sont pas affichés dans l’interface d’utilisation pour les instances envoyées, bien qu’ils soient présents dans le fichier XML stocké dans le référentiel CRX. NPR-17075
...
* Correctifs pour l’utilisation des audiences de ciblage-synchronisation, la publication de campagnes et la sélection de la mesure de l’objectif dans l’interface d’utilisation tactile
* Résout des problèmes de synchronisation pour les tâches de traduction
* Renforce la sécurité du service de préremplissage Forms
* Améliorations du composant d’envois et des brouillons du portail Formulaires, ainsi que du service Barcoded Forms
...
* Après la mise à niveau d’AEM 6.0 vers AEM 6.2, l’interface d’utilisation classique est lente lors de la recherche de balises en raison de nombreuses requêtes. Pour résoudre ce problème, vous pouvez suivre les étapes mentionnées sous [Désactiver le statut de réplication dans la console de balisage (interface d’utilisation classique)](#disable-replication-status-in-tagging-console-classic-ui-npr). NPR-15842 : correctif pour CQ-4201748.

* Lors de la création d’une page dans l’interface d’utilisation tactile, la vérification d’entrée pour le champ « nom » ne vérifie pas le caractère spécial « Apostrophe » (identique à celui de l’interface d’utilisation classique). La page ne peut donc pas être déplacée. NPR-16404 : correctif pour CQ-4205321.
...
* Sur l’écran Sites de l’interface d‘utilisation tactile, une tentative de collage d’une page à l’intérieur d’une page sans sous-page ne fonctionne pas car le bouton Coller n’apparaît pas. NPR-15894 : correctif pour CQ-4201696.
* Lors du défilement de l’onglet Pages du panneau Content Finder, quelques jeux de pages sont affichés indéfiniment dans l’interface d’utilisation classique, tandis que l’interface d’utilisation tactile présente un ensemble limité de pages non répétées. NPR-16271 : correctif pour CQ-4202371
...
* La personne ne peut pas télécharger de fichiers ni créer de dossiers lorsqu’elle navigue dans les ressources en raison d’un comportement incohérent lors de l’utilisation du bouton « Créer ». NPR-16410 : correctif pour CQ-4204793
...
* La tâche de traduction de synchronisation est à présent déclenchée automatiquement et l’interrogation d’AEM se produit sans interroger les projets de traduction. NPR-15163 : correctif pour CQ-90856
...
* Protection du service de préremplissage de formulaires contre un accès non autorisé par le biais de protocoles tels que `file://`, `http://` et `ftp://`. Consultez la section [Configurer le service de préremplissage à l’aide de Configuration Manager](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-15414
...
* Lorsque vous travaillez avec un fragment de texte dans une lettre en mode Aperçu ou Modification, si le texte est converti en liste, la fonctionnalité de lettre entière est sans effet. Une erreur JavaScript est également générée. NPR-16460
...
* L’ajout ou la modification d’un dictionnaire de données en fragment de document de texte existant prend beaucoup de temps et la personne à l’origine de l’opération n’est pas toujours informée. NPR-16102
* Lors de la prévisualisation d’une lettre avec du contenu défilant à l’aide du navigateur Internet Explorer, la barre de défilement du navigateur chevauche la barre de défilement de la lettre. Par conséquent, le contenu entier ne peut pas être affiché pour les fragments sur le côté droit. NPR-16068
* Lors de la création ou de la modification de fragments de document de texte à l’aide du navigateur Google Chrome, la liste déroulante de sélection de couleurs apparaît automatiquement et ne peut pas être supprimée. L’utilisateur ou l’utilisatrice doit sélectionner la liste comme type de saisie de données pour pouvoir modifier le fragment. NPR-16067
...
* Le type de liaison de données dans les lettres lors de la nouvelle création est affiché comme « Utilisateur ou utilisatrice » même s’il avait été défini différemment auparavant. NPR-16619
...
* Améliorez la fonctionnalité de recherche tout en prévisualisant une lettre dans Correspondence Management afin d’activer la recherche de texte dans le contenu du fragment de texte plutôt que de rechercher uniquement le libellé ou le titre de la lettre. NPR-16103
...
Les principaux aspects du pack de correctifs cumulatif 2 sont les suivants :
...
* La valeur pour les balises avec la propriété `tagBasePath` n’est pas récupérée dans la boîte de dialogue de l’interface d’utilisation classique lors de la navigation vers un composant de page. NPR-15543 : correctif pour CQ-4199950
...
* Une personne sélectionne un nœud dans CRXDE (par exemple, `/content/dam/`). Cliquez ensuite sur l’onglet Contrôle d’accès, en vous assurant qu’il existe une liste de contrôle d’accès. Désormais, le fait de faire glisser et de déposer certains éléments déplace les éléments et non ceux sélectionnés. NPR-15696 correctif pour GRANITE-16300
...
* Le statut de modification de la boîte de réception Workflow dans le noyau de gestion de contenu web ne change pas si la personne modifie une page après l’activation d’un workflow. NPR-15684 : correctif pour CQ-4196974
* Le module d’ancrage dans l’éditeur de texte enrichi pour l’interface d’utilisation tactile génère du code HTML5 non conforme lorsque la personne clique sur l’icône d’ancrage et ajoute un nom. Il doit ajouter un attribut « id » au lieu de « name » dans la balise HTML5 pour l’élément d’ancrage. NPR-15650 : correctif pour CQ-89782
...
* Dans l’interface d’utilisation de l’éditeur de fragments de contenu, si de nombreuses variantes d’un fragment de contenu sont créées, le panneau latéral n’affiche pas la barre de défilement pour parcourir toutes les variations. NPR-15445 : correctif pour CQ-4199444
...
* Le bouton Effacer est manquant dans l’onglet miniature des propriétés de la page du composant Sites. NPR-15143 correctif pour CQ-4196997
* Pour un site qui utilise des Live Copies, la sélection de la case à cocher « Live Copy » dans le volet Colonnes de la console d’administration du site ne permet pas d’afficher correctement le statut de Live Copy. Seule l’annotation HTML est affichée. NPR-15108 : correctif pour CQ-97086
...
* La fermeture de la page Modifier en mode Distorsion du temps suivie de la tentative de réouverture de la page à partir de l’administration de Site entraînent une erreur avec le statut `500`. NPR-14965 : correctif pour CQ-109647 :
...
* Après avoir recherché un fichier dans OmniSearch, la personne qui sélectionne une ressource et tente de modifier ses propriétés en cliquant sur **Afficher les propriétés**, puis sur le bouton **Enregistrer** est redirigée vers une page vierge. NPR-15900 : correctif pour CQ-4202372
...
* Lors de la publication d’une ressource en mode Carte, la carte n’est pas mise à jour pour refléter un état publié. La page est actualisée à la place. NPR-15826 : correctif pour CQ-102732
...
* Lors de la navigation parmi les ressources et de l’utilisation du mode d’affichage « Mode Colonnes », si la personne actualise la page après avoir sélectionné et cliqué sur une ressource, les détails de cette ressource apparaissent à la place du contenu actualisé. NPR-15768 : correctif pour CQ-4201727
...
* L’accès à l’interface d’utilisation « My Link Shares » avec le navigateur Firefox ne permet pas d’afficher les personnes et éléments partagés ; l’écran est inutilisable. NPR-15539 : correctif pour CQ-4200992
* Lors de l’utilisation de la gestion des ressources numériques, si une page est associée à un ensemble d’images, le déplacement des images vers un nouveau dossier rompt l’association des pages. Par conséquent, certaines des images manquent dans la page associée. NPR-15538 : correctif pour CQ-111479
...
* Lors de l’utilisation de l’outil de recherche de ressources en mode Colonnes, une tentative de création de dossier échoue la première fois, mais fonctionne lors de l’essai suivant. NPR-15370 : correctif pour CQ-4199448
...
* Dans l’**[!UICONTROL interface d’utilisation tactile]**, la date enregistrée est affichée pour les champs de date qui n’ont pas de type=&#39;datetime&#39; lors de l’utilisation du navigateur Internet Chrome version 56.0.2924.87. NPR-15383 : correctif pour GRANITE-16481
...
* OmniSearch ne permet pas à la clientèle qui utilise `/apps` d’ajouter une source de données de colonne, car la configuration de l’emplacement est supposée être répertoriée sous */libs/granite/omnisearch/content/metadata/*. NPR-13188 correctif pour GRANITE-16479
...
* Lors de l’utilisation des composants d’intégration Campaign - Ciblage et `MAC` - Test et Target, l’annulation de la publication des activités ne met pas à jour le statut de l’activité dans l’interface d’utilisation principale. NPR-15401 : correctif pour CQ-4199839
...
* Le service de préremplissage ne fonctionne pas pour les formulaires XDP. NPR-15466
...
* Lors de l’utilisation du service Output pour fusionner un formulaire spécifique avec un contenu dataxml, le temps de réponse est 20 fois plus long que pour le serveur ES4 SP1. NPR-15283
...
* Les personnes ne peuvent pas se connecter à l’application AEM Forms. NPR-15891
...
* Les listes déroulantes de cibles ne fonctionnent pas dans la boîte de dialogue de l’interface d’utilisation tactile lorsque plusieurs champs de texte enrichi, tels que des liens, sont créés. NPR-13911
...
* Lors de l’envoi de la boîte de dialogue Propriétés de la page depuis l’éditeur, le processeur de publication MSM écrit certains paramètres de la requête au lieu du paramètre `msm:writeLiveCopyConfig`. NPR-14434
...
* Requêtes de recherche de ressources exécutées lentement dans l’interface d’utilisation tactile pendant des recherches simultanées effectuées par plusieurs personnes. NPR-15019 : correctif pour CQ-4195405
...
* Les personnes ne peuvent pas supprimer une recherche enregistrée ou une collecte dynamique. NPR-14924 : correctif pour CQ-108494
...
* Les personnes ne disposant pas de droits de réplication peuvent désormais supprimer les dossiers de ressources. NPR-14321 : correctif pour CQ-88271
...
* Les personnes ne peuvent pas modifier les métadonnées des ressources en bloc à partir de la page de propriétés. NPR-13721 : correctif pour CQ-100703
...
* Les personnes de l’instance de création AMS AEM Assets y accèdent depuis différents réseaux qui ne permettent pas de charger des ressources en bloc sans disposer de droits de suppression sur les dossiers. NPR-12768 : correctif pour CQ-82715
...
* Depuis la mise à jour de la requête `Jquery`, les utilisateurs et utilisatrices ne peuvent pas sélectionner une collection de ressources et confirmer la sélection dans le panneau Associer le contenu d’un fragment de contenu. NPR-14847 : rétroportage pour CQ-4194209
...
* Lors de l’export d’un projet de traduction créé avec la méthode de traduction « humaine » et le fournisseur de traduction « aucun », aucun fichier translation_export_summary.xml n’est généré en raison de l’absence de fichier de mappage du GUID. NPR-13137 : correctif pour CQ-91976
...
* Le lien Payload dans l’onglet Archivage de la console Workflow renvoie une erreur avec le code de réponse « 404 » lorsque l’on clique dessus. NPR-14993 : correctif pour CQ-4194977
...
* Les personnes ne peuvent pas prévisualiser le payload à partir de la console de la boîte de réception de workflow lors de l’initialisation de workflows. NPR-13226 : correctif pour CQ-100275
...
* Lors de la navigation dans le répertoire *FormsandDocuments*, le bouton Coller n’apparaît pas lorsque la personne copie une ressource puis accède à un autre dossier. CQ-111327

#### Programme d’installation de Forms JEE {#forms-jee-installer-19}

`Rights Management`

* L’événement d’audit associé à la connexion de l’utilisateur ou de l’utilisatrice est consigné avec une heure non valide. Le moment approprié pour l’événement d’audit n’est pas traçable. NPR-13107
...
* Lorsqu’une tâche de brouillon transférée dans l’espace de travail HTML est renvoyée à la personne initiale, cette tâche n’apparaît pas dans sa file d’attente. NPR-15178
...
* Forms Designer se bloque lorsque la personne exécute l’utilitaire de vérification orthographique avec le français (Canada) sélectionné comme langue de formulaire. NPR-13740
...
>[!NOTE]
>
>Pour les clientes et clients AEM Forms, il est impératif d’installer le package complémentaire AEM Forms après l’installation de tout pack de services, pack de services cumulatif ou pack de fonctionnalités d’AEM.
...
* Pour un déploiement en cluster utilisant RDBMK ou MongoDB, le package CFP peut être installé sur n’importe quelle instance de création qui utilise le gestionnaire de modules.
...
1. Tous les lots OSGI sont répertoriés comme ACTIFS ou FRAGMENTS dans la console OSGI (utiliser la console web : /system/console/bundles).
...
Si vous utilisez CFP3 ou une version ultérieure, suivez ces instructions pour désactiver le statut de réplication dans la console de balisage de l’interface d’utilisation classique :
...
>[!NOTE]
>
>Ces paramètres de configuration sont spécifiques à la clientèle AEM Forms qui utilise Document Security.

NPR-21355 est inclus dans le CFP12.1. Si vous installez le CFP 12.1 ou une version ultérieure, effectuez la procédure ci-dessous pour configurer NPR-21355 sur le serveur d’applications JBoss®. Si vous installez CFP 12.1 sur un serveur AEM Forms qui s’exécute sur des serveurs d’applications Oracle WebLogic ou IBM® WebSphere, aucune configuration supplémentaire n’est nécessaire :

1. sauvegardez, supprimez et créez le fichier module.xml. L’emplacement par défaut du fichier est [Répertoire_d_installation_d_AEM_Forms]/jboss/modules/system/couches/base/com/adobe/livecycle/main/
...
1. Créez une sauvegarde des fichiers `jsafeFIPS.jar`, `jsafeJCEFIPS.jar` et `certjFIPS.jar` à l’emplacement [AEM_Forms_Installation_directory]`/jboss/modules/system/layers/base/com/adobe/livecycle/main/` et supprimez les fichiers du répertoire mentionné précédemment.
...
La valeur pour la file d’attente partagée n’est pas actualisée pour les autres personnes lorsqu’une personne demande une tâche. Adobe a créé une nouvelle propriété pour cela. Suivez les étapes ci-dessous pour configurer cette propriété sur votre instance AEM :
...
* Une erreur interne de serveur HTTP 500 est reçue à l’ouverture de la page des détails du composant de console web.
...
* Lorsque vous installez CFPx sur AEM 6.2 SP1 avec le pack de fonctionnalités Balises intelligentes, l’étape de workflow ajoutée précédemment pour les ressources de balises intelligentes est supprimée du workflow de mise à jour des ressources DAM.
